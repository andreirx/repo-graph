...
describe('chunk rollup ordering and determinism', () => {
  ...
  it('whole_file mode writes marker comment and body, no frontmatter', async () => {
    ...
    // No frontmatter (operator-directed, 2026-07-10): the provenance block
    // was a per-read token tax duplicating body/git information. The file
    // starts with the generated-marker comment, then the body — nothing else.
    const matter = await import('gray-matter');
    const { data: frontmatter, content: body } = matter.default(content);

    expect(content.startsWith('<!-- generated by rgistr; do not hand-edit -->\n')).toBe(true);
