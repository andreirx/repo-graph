...
pub fn validate_quality_policy_payload(
    ...
    let measurement_kind_parsed = SupportedMeasurementKind::parse(&payload.measurement_kind);
...
pub fn parse_measurement_kind(
    kind: &str,
) -> Result<SupportedMeasurementKind, PolicyValidationError> {
    SupportedMeasurementKind::parse(kind).ok_or_else(|| {
        PolicyValidationError::UnknownMeasurementKind {
            kind: kind.to_string(),
        }
    })
}
...
mod tests {
    ...
    fn measurement_kind_roundtrip() {
        for kind in SupportedMeasurementKind::all() {
            let s = kind.as_str();
            let parsed = SupportedMeasurementKind::parse(s);
    ...
    fn measurement_kind_unknown_returns_none() {
        assert_eq!(SupportedMeasurementKind::parse("unknown_metric"), None);
        assert_eq!(SupportedMeasurementKind::parse(""), None);
        assert_eq!(
            SupportedMeasurementKind::parse("CYCLOMATIC_COMPLEXITY"),
            None
        ); // case sensitive
    }
    ...
    fn parse_measurement_kind_valid() {
        ...
        assert_eq!(
            result.unwrap(),
            SupportedMeasurementKind::CyclomaticComplexity
        );
    }
    ...
    fn suggested_policy_kinds_non_empty() {
        for kind in SupportedMeasurementKind::all() {
            ...
            assert!(!suggestions.is_empty(), "no suggestions for {kind:?}");
        }
    }
    ...
    fn validate_complexity_with_comparative_allowed() {
        ...
        assert!(
            errors.is_empty(),
            "no_worsened should be allowed for complexity"
        );
    }
}
