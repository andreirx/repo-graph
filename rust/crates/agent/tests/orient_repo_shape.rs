...
use repo_graph_agent::{orient, Budget, OrientError, ORIENT_COMMAND, ORIENT_SCHEMA};
...
fn repo_orient_serializes_to_contract_shape() {
    ...
    assert!(
        json["focus"].get("resolved_key").is_none(),
        "repo focus must not have resolved_key"
    );
    assert!(
        json["focus"].get("resolved_path").is_none(),
        "repo focus must not have resolved_path"
    );
