...
use repo_graph_agent::{
    orient, AgentReliabilityAxis, AgentReliabilityLevel, AgentStaleFile, AgentTrustSummary, Budget,
    Confidence, EnrichmentState,
};
...
fn high_confidence_when_rate_high_and_enrichment_ran() {
    let fake = with_trust(make_trust(0.80, EnrichmentState::Ran, 10, 9), false);
...
fn medium_confidence_when_rate_in_band() {
    let fake = with_trust(make_trust(0.30, EnrichmentState::Ran, 10, 9), false);
    let result = orient(&fake, "r1", None, Budget::Small, common::TEST_NOW).unwrap();
...
fn low_confidence_when_rate_below_20_percent() {
    let fake = with_trust(make_trust(0.10, EnrichmentState::Ran, 10, 9), false);
    let result = orient(&fake, "r1", None, Budget::Small, common::TEST_NOW).unwrap();
...
fn high_rate_degrades_to_medium_when_stale() {
    let fake = with_trust(make_trust(0.80, EnrichmentState::Ran, 10, 9), true);
    let result = orient(&fake, "r1", None, Budget::Small, common::TEST_NOW).unwrap();
    assert_eq!(result.confidence, Confidence::Medium);
}
...
fn high_rate_degrades_to_medium_when_enrichment_not_run() {
    ...
    let fake = with_trust(make_trust(0.80, EnrichmentState::NotRun, 0, 0), false);
    let result = orient(&fake, "r1", None, Budget::Small, common::TEST_NOW).unwrap();
    assert_eq!(result.confidence, Confidence::Medium);
}
...
fn high_rate_stays_high_when_enrichment_ran_with_zero_enriched() {
    ...
    let fake = with_trust(make_trust(0.80, EnrichmentState::Ran, 10, 0), false);
    let result = orient(&fake, "r1", None, Budget::Small, common::TEST_NOW).unwrap();
    assert_eq!(result.confidence, Confidence::High);
}
