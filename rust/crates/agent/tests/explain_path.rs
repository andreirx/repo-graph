...
use repo_graph_agent::{
    run_explain, AgentFileEntry, AgentPathResolution, AgentRepoSummary, Budget, SignalCode,
    EXPLAIN_COMMAND,
};
...
fn explain_path_has_identity_section() {
    ...
    let result = run_explain(&fake, "r1", "src/core", Budget::Medium, TEST_NOW).unwrap();
...
fn explain_path_files_truncated_at_cap() {
    ...
    let files: Vec<AgentFileEntry> = (0..20)
        ...
        .collect();
    fake.files_in_path
        .insert(("snap1".into(), "src/core".into()), files);
...
fn explain_path_no_match_returns_empty() {
    ...
    let result = run_explain(&fake, "r1", "nonexistent/path", Budget::Medium, TEST_NOW).unwrap();
