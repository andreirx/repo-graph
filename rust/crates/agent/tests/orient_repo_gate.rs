...
use repo_graph_agent::{orient, Budget, LimitCode, SignalCode, SignalEvidence};
use repo_graph_gate::{
    GateBoundaryDeclaration, GateImportEdge, GateMeasurement, GateObligation, GateRequirement,
};
...
fn gate_pass_emitted_when_arch_violations_clean() {
    ...
    fake.gate_requirements
        .insert("r1".into(), vec![arch_requirement()]);
...
fn finite_waiver_applies_before_expiry_at_orient_level() {
    ...
    let result = orient(&fake, "r1", None, Budget::Medium, "2026-04-15T00:00:00Z").unwrap();
    let sig = find_signal(&result, SignalCode::GatePass)
        .expect("active waiver before expiry must suppress FAIL and yield GATE_PASS");
    match sig.evidence() {
        SignalEvidence::GatePass(ev) => {
            assert_eq!(ev.waived_count, 1);
            assert_eq!(ev.pass_count, 0);
...
fn finite_waiver_does_not_apply_after_expiry_at_orient_level() {
    ...
    let result = orient(&fake, "r1", None, Budget::Medium, "2026-09-01T00:00:00Z").unwrap();
    let sig = find_signal(&result, SignalCode::GateFail)
        .expect("expired waiver must NOT suppress FAIL; GATE_FAIL must be reported");
    match sig.evidence() {
        SignalEvidence::GateFail(ev) => {
            assert_eq!(ev.fail_count, 1);
            assert_eq!(ev.total_count, 1);
...
fn perpetual_waiver_applies_regardless_of_now() {
    ...
    for now in &["2020-06-01T00:00:00Z", "9999-01-01T00:00:00Z"] {
        ...
        let sig = find_signal(&result, SignalCode::GatePass)
            .unwrap_or_else(|| panic!("perpetual waiver must apply at now={}", now));
        match sig.evidence() {
