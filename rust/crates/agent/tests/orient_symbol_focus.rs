...
use repo_graph_agent::{
    orient, AgentFocusCandidate, AgentFocusKind, AgentPathResolution, AgentSymbolContext, Budget,
    FocusFailureReason, ResolvedKind,
};
...
fn symbol_resolves_by_stable_key() {
    ...
    assert_eq!(result.focus.resolved_path.as_deref(), Some("src/lib.rs"));
}
...
fn symbol_name_multiple_matches_returns_ambiguous() {
    ...
    let result = orient(&fake, "r1", Some("init"), Budget::Small, common::TEST_NOW).unwrap();

    assert!(!result.focus.resolved, "ambiguous must not resolve");
    assert_eq!(result.focus.reason, Some(FocusFailureReason::Ambiguous));
    assert_eq!(
    ...
    assert!(result.limits.is_empty(), "ambiguous must emit zero limits");
}
...
fn symbol_name_zero_matches_returns_no_match() {
    ...
    assert!(!result.focus.resolved);
    assert_eq!(result.focus.reason, Some(FocusFailureReason::NoMatch));
    assert_eq!(result.focus.input.as_deref(), Some("nonExistentSymbol"));
