...
use repo_graph_gate::{
    ...
    GateModuleViolationEvidence, GateRequirement, GateStorageError, GateStorageRead, GateWaiver,
};
...
pub struct FakeAgentStorage {
    ...
    pub imports_between_paths: HashMap<(String, String, String), Vec<AgentImportEdge>>,
    ...
    pub boundary_declarations_in_path: HashMap<(String, String), Vec<AgentBoundaryDeclaration>>,
    ...
    pub gate_boundary_imports: HashMap<(String, String, String), Vec<GateImportEdge>>,
...
impl FakeAgentStorage {
    ...
    pub fn seed_minimal_repo(&mut self, repo_uid: &str, repo_name: &str, snapshot_uid: &str) {
        self.repos.insert(
            repo_uid.to_string(),
            AgentRepo {
        ...
        self.trust_summaries
            .insert(snapshot_uid.to_string(), high_confidence_trust());
    }

    fn fail_if_forced(&self, operation: &'static str) -> Result<(), AgentStorageError> {
...
impl AgentStorageRead for FakeAgentStorage {
    ...
    fn find_symbol_callers(
        ...
        let key = (snapshot_uid.to_string(), symbol_stable_key.to_string());
        Ok(self.symbol_callers.get(&key).cloned().unwrap_or_default())
    }
    ...
    fn list_symbols_in_file(
        ...
        let key = (snapshot_uid.to_string(), file_path.to_string());
        Ok(self.symbols_in_file.get(&key).cloned().unwrap_or_default())
    }
    ...
    fn list_files_in_path(
        ...
        let key = (snapshot_uid.to_string(), path_prefix.to_string());
        Ok(self.files_in_path.get(&key).cloned().unwrap_or_default())
    }
...
impl FakeAgentStorage {
    fn fail_if_forced_gate(&self, operation: &'static str) -> Result<(), GateStorageError> {
        if let Some(op) = *self.force_error_on.borrow() {
            if op == operation {
...
impl GateStorageRead for FakeAgentStorage {
    ...
    fn find_waivers(
        ...
        let all = self.gate_waivers.get(&key).cloned().unwrap_or_default();
