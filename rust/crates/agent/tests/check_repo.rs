...
fn no_snapshot_produces_incomplete() {
    ...
    match sig.evidence() {
        SignalEvidence::CheckIncomplete(ev) => {
            ...
            assert_eq!(ev.incomplete_conditions[0].code, "SNAPSHOT_EXISTS");
            assert!(ev.fail_conditions.is_empty());
            assert!(ev.passing.is_empty());
        }
        other => panic!("expected CheckIncomplete evidence, got {:?}", other),
    }
...
fn stale_files_produce_fail() {
    ...
    let sig = find_signal(&result, SignalCode::CheckFail).expect("expected CHECK_FAIL signal");
    match sig.evidence() {
        SignalEvidence::CheckFail(ev) => {
            let codes: Vec<&str> = ev.fail_conditions.iter().map(|c| c.code.as_str()).collect();
            assert!(
        ...
        other => panic!("expected CheckFail evidence, got {:?}", other),
    }
}
...
fn call_graph_medium_everything_pass() {
    ...
    let sig = find_signal(&result, SignalCode::CheckPass).expect("expected CHECK_PASS signal");
    match sig.evidence() {
        ...
        other => panic!("expected CheckPass evidence, got {:?}", other),
    }
}
    ...
    match sig.evidence() {
        SignalEvidence::CheckIncomplete(ev) => {
            let incomplete_codes: Vec<&str> = ev
                ...
                .map(|c| c.code.as_str())
                .collect();
            let fail_codes: Vec<&str> =
                ev.fail_conditions.iter().map(|c| c.code.as_str()).collect();
            assert!(
...
fn snapshot_info_emitted_when_snapshot_exists() {
    ...
    let sig =
        find_signal(&result, SignalCode::SnapshotInfo).expect("expected SNAPSHOT_INFO signal");
    match sig.evidence() {
        SignalEvidence::SnapshotInfo(ev) => {
            assert_eq!(ev.snapshot_uid, "snap-1");
        }
        other => panic!("expected SnapshotInfo evidence, got {:?}", other),
    }
}
...
fn check_emits_exactly_two_signals_when_snapshot_exists() {
    ...
    assert_eq!(
        ...
        result.signals.iter().map(|s| s.code()).collect::<Vec<_>>()
    );
}
