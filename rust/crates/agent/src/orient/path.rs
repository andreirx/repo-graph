...
pub fn orient_path<S: AgentStorageRead + GateStorageRead + ?Sized>(
    ...
    let boundary_out =
        aggregators::boundary::aggregate_path(storage, repo_uid, snapshot_uid, path_prefix)?;
    ...
    let mod_out = aggregators::module_summary::aggregate_path(storage, snapshot_uid, path_prefix)?;
    merge(&mut all_signals, &mut all_limits, mod_out);

    // ── gate (path-scoped) ──────────────────────────────────
    let gate_out =
        aggregators::gate::aggregate_path(storage, repo_uid, snapshot_uid, now, path_prefix)?;
