...
fn build_documentation_section<S: AgentStorageRead + ?Sized>(
    ...
    //
    // ORIENT-SEGMENT-2 §2.6: consume SELF-POLLUTION-1's classifier — rmap's own
    // self-generated map sidecars (`generated == true`, marker-evidenced) are exhaust,
    // never primary orientation, so they are EXCLUDED from orient's Docs line (the same
    // default `docs list` applies). `.env*` is already dropped upstream in the doc
    // inventory. The `generated` flag is EVIDENCE-based (the `[generated by rmap map]`
    // first-line marker), so a user's marker-less `MAP.md` name-collision is NOT
    // excluded — honesty preserved. Repo-level orient only: file/module focus keeps its
    // own doc selection, where a focus-path MAP.md can be the most relevant doc.
