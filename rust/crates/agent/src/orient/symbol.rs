...
pub fn orient_symbol<S: AgentStorageRead + GateStorageRead + ?Sized>(
    ...
    if let Some(ref module_path) = context.module_path {
        ...
        let boundary_out =
            aggregate_boundary_for_module(storage, repo_uid, snapshot_uid, module_path)?;
        ...
        let gate_out =
            aggregate_gate_for_module(storage, repo_uid, snapshot_uid, now, module_path)?;
    ...
    let documentation =
        build_documentation_section(storage, repo_uid, context.file_path.as_deref());
...
fn project_report(report: &repo_graph_gate::GateReport) -> Vec<Signal> {
    use crate::dto::signal::{GateFailEvidence, GateIncompleteEvidence, GatePassEvidence};
    ...
    match report.outcome.outcome.as_str() {
        ...
        "incomplete" => {
            vec![Signal::gate_incomplete(GateIncompleteEvidence {
...
fn map_gate_error(e: repo_graph_gate::GateError) -> OrientError {
    match e {
        repo_graph_gate::GateError::Storage(inner) => OrientError::Storage(
            crate::errors::AgentStorageError::new(inner.operation, inner.message),
        ),
        repo_graph_gate::GateError::MalformedEvidence { operation, reason } => {
            OrientError::Storage(crate::errors::AgentStorageError::new(operation, reason))
        }
    }
}
