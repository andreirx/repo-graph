...
//!   - medium: resolution rate in [0.20, 0.50], OR rate > 0.50 AND stale,
//!     OR rate > 0.50 AND enrichment_state is NotRun
//!   - high: rate > 0.50 AND not stale AND enrichment state is Ran or
//!     NotApplicable
//!
...
pub fn derive_repo_confidence(trust: &AgentTrustSummary, stale: bool) -> Confidence {
...
mod tests {
    ...
    fn high_rate_with_not_applicable_enrichment_stays_high() {
        ...
        // to `NotApplicable`.
        assert_eq!(
            derive_repo_confidence(&ts(0.80, EnrichmentState::NotApplicable, 0), false),
            Confidence::High
        );
    }
}
