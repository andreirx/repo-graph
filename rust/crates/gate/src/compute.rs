...
use crate::types::{
    EffectiveVerdict, GateAssessmentState, GateAssessmentVerdict, GateCounts, GateMode,
    GateObligation, GateOutcome, GateQualityAssessmentEvaluation, GateQualityAssessmentFact,
    GateQualityCounts, GateQualityPolicySeverity, GateReport, GateRequirement, GateWaiver,
...
fn evaluate_obligation(
    ...
    match evidence {
        ...
        MethodEvidence::CoverageThreshold {
            ...
            evaluate_coverage_threshold(&mut eval, matching_measurements, &op, threshold);
        }
        ...
        MethodEvidence::ComplexityThreshold {
            ...
            let op = obl.operator.as_deref().unwrap_or("<=").to_string();
            let threshold = obl.threshold.unwrap_or(0.0);
            evaluate_complexity_threshold(&mut eval, matching_measurements, &op, threshold);
        }
...
fn evaluate_arch_violations(
    ...
    let total_violations: usize = per_boundary_counts.iter().map(|(_, n)| *n).sum();
...
fn reduce_outcome(
        ...
        match fact.assessment_state {
            ...
            GateAssessmentState::Present => {
                match fact.computed_verdict {
                    ...
                    Some(GateAssessmentVerdict::Fail) => match fact.severity {
    ...
    let has_quality_incomplete = quality_counts.missing > 0 || quality_counts.not_comparable > 0;
...
mod tests {
    ...
    fn arch_violations_missing_when_no_boundary_for_target() {
        ...
        let report = compute(input);
        assert_eq!(
            report.obligations[0].computed_verdict,
    ...
    fn coverage_threshold_missing_when_target_missing() {
        let r = req(
            "REQ-1",
            1,
            vec![obl("o1", "coverage_threshold", None, Some(0.80))],
        );
        let mut input = empty_input(vec![r], GateMode::Default);
        input
            .method_evidence
    ...
    fn hotspot_threshold_pass_with_no_target() {
        let r = req(
            "REQ-1",
            1,
            vec![obl("o1", "hotspot_threshold", None, Some(0.80))],
        );
        let mut input = empty_input(vec![r], GateMode::Default);
        input.method_evidence.insert(
            key("REQ-1", 1, "o1"),
    ...
    fn module_violations_fail_when_violations_exist() {
        ...
        assert_eq!(
            report.obligations[0].evidence["stale_declarations_count"],
            1
        );
        assert_eq!(report.outcome.exit_code, 1);
    }
    ...
    fn module_violations_ignores_target_field_on_obligation() {
        ...
        let r = req(
            "REQ-1",
            1,
            vec![obl("o1", "module_violations", Some("packages/app"), None)],
        );
        let mut input = empty_input(vec![r], GateMode::Default);
        input.method_evidence.insert(
            key("REQ-1", 1, "o1"),
