...
use crate::compute::{compute, GateInput, MethodEvidence, ObligationKey, PolicyMeasurement};
use crate::errors::GateError;
use crate::storage_port::GateStorageRead;
use crate::types::{GateInference, GateMeasurement, GateMode, GateReport, GateRequirement};
    ...
    I: IntoIterator<Item = GateMeasurement>,
{
    ...
    for row in rows {
        let parsed: serde_json::Value =
            serde_json::from_str(&row.value_json).map_err(|e| GateError::MalformedEvidence {
        ...
        let value = parsed["value"]
            .as_f64()
            .ok_or_else(|| GateError::MalformedEvidence {
                operation: kind.operation(),
                reason: format!(
...
pub fn assemble_from_requirements<S: GateStorageRead + ?Sized>(
        ...
        for obl in &req.obligations {
            ...
            let evidence: MethodEvidence = match obl.method.as_str() {
                ...
                "module_violations" => {
                    ...
                    let evidence = storage.evaluate_module_violations(repo_uid, snapshot_uid)?;
    ...
    let mut pending_waivers: Vec<(ObligationKey, Vec<crate::types::GateWaiver>)> = Vec::new();
    for req in &input.requirements {
        for obl in &req.obligations {
            let key = ObligationKey::new(&req.req_id, req.version, &obl.obligation_id);
...
mod tests {
    ...
    use crate::types::{
        ...
        GateModuleViolationEvidence, GateObligation, GateRequirement, GateWaiver, Verdict,
    };
