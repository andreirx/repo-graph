...
pub fn detect_react_hooks(file_inputs: &[FileInput]) -> Vec<ReactHookDetection> {
	...
	for file in file_inputs {
		...
		let file_hooks = detect_hooks_in_file(
			&tree.root_node(),
			file.content.as_bytes(),
			&file.rel_path,
		);
...
fn collect_components(
	...
	if node.kind() == "function_declaration" {
		if let Some(component) = try_extract_function_component(node, source, file_path, import_gate) {
...
fn try_extract_arrow_component(
	...
	let has_fc_type = has_fc_type_annotation(&declarator, source)
		|| has_fc_type_in_value(&value, source);
	...
	let has_jsx = arrow_body.map(|b| contains_jsx_return(&b, source)).unwrap_or(false);
...
fn contains_jsx_return(body: &Node, source: &[u8]) -> bool {
	...
	if body.kind() == "jsx_element" || body.kind() == "jsx_self_closing_element" || body.kind() == "jsx_fragment" {
		return true;
	}
...
fn detect_hooks_in_file(
	root: &Node,
	source: &[u8],
	file_path: &str,
) -> Vec<ReactHookDetection> {
	let mut hooks = Vec::new();
	let mut enclosing_component: Option<String> = None;
	collect_hooks(root, source, file_path, &mut enclosing_component, &mut hooks);
	hooks
}
