//! Spring `@RestController` HTTP PROVIDER detection (Java).
//!
//! Joins each method-level mapping annotation to its enclosing
//! `@RestController` class's `@RequestMapping` base path via `parent_node_uid`,
//! composing (verb, base + suffix). Reuses the RAW annotation parser exposed by
...
/// Detect Spring `@RestController` provider routes from extracted Java nodes.
///
/// `provider_files` is the set of repo-relative `.java` files that import the
/// Spring web annotation package — the IMPORT evidence a `@RestController`
...
pub(super) fn detect_spring_http_providers(
    ...
    // Pass 1: rest-controller classes → base path, keyed by node_uid.
    let mut controller_base: HashMap<&str, BasePath> = HashMap::new();
    for node in nodes {
        let anns = parse_node_annotations(node.metadata_json.as_deref());
        if anns.is_empty() {
            continue;
        }
        let is_rest_controller = anns.iter().any(|a| a.simple_name == "RestController");
        if !is_rest_controller {
            continue;
        }
        // IMPORT evidence: the controller's file must import the Spring web
        // annotation package. Without it, the `@RestController` name is not a
        // trustworthy Layer-3 fact.
        ...
        controller_base.insert(node.node_uid.as_str(), base);
    }
    ...
    for node in nodes {
        ...
        let base = match controller_base.get(parent) {
            Some(b) => b,
            None => continue,
        };
        let anns = parse_node_annotations(node.metadata_json.as_deref());
        for ann in &anns {
            ...
            let route = match (base, read_path_arg(ann.args_raw.as_deref())) {
                // Base or method path unreadable → the composed route is UNKNOWN.
                (BasePath::Unknown, _) => None,
                (_, PathArg::Unreadable) => None,
                (BasePath::Known(b), PathArg::Literal(s)) => Some(join_route(b, &s)),
                (BasePath::Known(b), PathArg::Absent) => Some(join_route(b, "")),
            };
            let (line, col) = node
                .location
                .as_ref()
...
mod tests {
    ...
    fn mapping_verb_maps_annotations() {
        ...
        assert_eq!(mapping_verb("RequestMapping", Some("(\"/x\")")), None);
