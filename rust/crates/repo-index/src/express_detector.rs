...
pub fn detect_express_routes(file_inputs: &[FileInput]) -> Vec<ExpressRouteDetection> {
	...
	for file in file_inputs {
		...
		let file_routes = detect_routes_in_file(
			&tree.root_node(),
...
fn detect_routes_in_file(root: &Node, source: &[u8], file_path: &str) -> Vec<ExpressRouteDetection> {
	let mut routes = Vec::new();
...
fn try_extract_route(call_node: &Node, source: &[u8], file_path: &str) -> Option<ExpressRouteDetection> {
