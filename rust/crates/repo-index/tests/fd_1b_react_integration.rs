...
"#;
	...
	assert_eq!(
		...
		component_inferences.iter().map(|i| &i.target_stable_key).collect::<Vec<_>>()
	);

	// Verify component names are in stable keys.
	let keys: Vec<&str> = component_inferences.iter().map(|i| i.target_stable_key.as_str()).collect();
...
"#;
	...
	assert!(component_inferences[0].target_stable_key.contains("RealComponent"));
}
