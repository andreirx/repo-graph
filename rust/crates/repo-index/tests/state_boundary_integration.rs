...
"#;
    ...
    let fs_path_nodes: Vec<_> = nodes.iter().filter(|n| n.kind == "FS_PATH").collect();
    assert_eq!(
        fs_path_nodes.len(),
        1,
...
"#;
    ...
    // Full index.
    let idx =
        index_path(&repo, &db_path, "myservice", &ComposeOptions::default()).expect("full index");
    ...
    let ref_result =
        refresh_path(&repo, &db_path, "myservice", &ComposeOptions::default()).expect("refresh");
...
"#;
    let (_dir, repo) = temp_repo(&[("src/a.ts", src_a), ("src/b.ts", src_b)]);
    let db_dir = tempfile::tempdir().unwrap();
    let db_path = db_dir.path().join("test.db");
...
"#;
    ...
    assert!(
        ...
        "stale orphan resource /etc/b should persist until full reindex, got: {:?}",
        names
    );
}
