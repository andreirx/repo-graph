...
"#;
    ...
    let (_dir, repo) = temp_repo(&[("package.json", package_json), ("src/index.ts", source)]);
    ...
    let get_surface = surfaces
        .iter()
        .find(|s| s.display_name.as_deref() == Some("GET /api/users"));
    assert!(get_surface.is_some(), "GET /api/users surface should exist");

    let post_surface = surfaces
        .iter()
        .find(|s| s.display_name.as_deref() == Some("POST /api/users"));
    assert!(
        post_surface.is_some(),
        "POST /api/users surface should exist"
    );
...
"#;
    ...
    let (_dir, repo) = temp_repo(&[("package.json", package_json), ("app.ts", source)]);
    let db_dir = tempfile::tempdir().unwrap();
    let db_path = db_dir.path().join("test.db");
...
"#;
    ...
    let app_surface = surfaces
        .iter()
        .find(|s| s.display_name.as_deref() == Some("GET /app-route"));
    let app2_surface = surfaces
        .iter()
        .find(|s| s.display_name.as_deref() == Some("GET /app2-route"));
