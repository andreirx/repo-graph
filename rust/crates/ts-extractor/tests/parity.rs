...
fn normalize_result(
    result: &repo_graph_indexer::types::ExtractionResult,
    nodes_list: &[repo_graph_indexer::types::ExtractedNode],
) -> Value {
    // Build stable_key → node_uid map for edge source resolution.
    let uid_to_key: BTreeMap<String, String> = nodes_list
        .iter()
    ...
    let mut edges: Vec<Value> = result
        .edges
        .iter()
        .map(|e| {
            let source_key = uid_to_key
                .get(&e.source_node_uid)
                .cloned()
                .unwrap_or_else(|| e.source_node_uid.clone());
            let edge_type = serde_json::to_value(e.edge_type).unwrap_or(Value::Null);
            let metadata: Value = e
                .metadata_json
                .as_ref()
                .and_then(|s| serde_json::from_str(s).ok())
                .unwrap_or(Value::Null);
            let loc_val = e.location.map(|l| {
                serde_json::json!({
                    "lineStart": l.line_start,
                    "colStart": l.col_start,
                    "lineEnd": l.line_end,
    ...
    let mut bindings: Vec<Value> = result
        .import_bindings
        .iter()
        .map(|b| {
            let loc_val = b.location.map(|l| {
                serde_json::json!({
                    "lineStart": l.line_start,
                    "colStart": l.col_start,
                    "lineEnd": l.line_end,
...
fn run_fixture(fixture: &Fixture) -> Result<Value, String> {
    ...
    let file_path = fixture.input["filePath"]
        .as_str()
    ...
    let snapshot_uid = fixture.input["snapshotUid"]
        .as_str()
        .ok_or("missing snapshotUid")?;

    let mut ext = TsExtractor::new();
    ext.initialize().map_err(|e| format!("init: {}", e))?;
    let result = ext
        .extract(source, file_path, file_uid, repo_uid, snapshot_uid)
...
fn parity_against_shared_ts_extractor_fixture_corpus() {
    ...
    if !generated.is_empty() {
        eprintln!("Generated {}: {:?}", generated.len(), generated);
    }
    if !failures.is_empty() {
        panic!("{} failed:{}", failures.len(), failures.join("\n"));
    }
    if !generated.is_empty() {
        panic!("Generated {}. Re-run.", generated.len());
    }
}
