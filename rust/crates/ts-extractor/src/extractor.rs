...
impl ExtractorPort for TsExtractor {
    ...
    fn initialize(&mut self) -> Result<(), ExtractorError> {
        ...
        parser
            .set_language(&self.tsx_language)
            .map_err(|e| ExtractorError {
    ...
    fn extract(
        ...
        for node in &mut ctx.nodes {
            if node.kind == NodeKind::Symbol && ctx.exported_names.contains(&node.name) {
...
fn extract_lexical_declaration(
    ...
    for child in node.children(&mut decl_cursor) {
        ...
        let graph_node = make_symbol_node(name, subtype, visibility, None, &child, source, ctx);
...
fn extract_class(node: &tree_sitter::Node, source: &[u8], exported: bool, ctx: &mut ExtractionCtx) {
    let name_node = match node.child_by_field_name("name") {
        Some(n) => n,
        None => return,
    };
...
fn extract_method(
    ...
    let params = node.child_by_field_name("parameters");
    let sig = params.map(|p| format!("{}{}", name, p.utf8_text(source).unwrap_or("()")));
        ...
        for j in 0..child.child_count() {
            ...
            for k in 0..clause.child_count() {
                ...
                ctx.edges.push(ExtractedEdge {
                    ...
                    metadata_json: Some(serde_json::json!({"targetName": iface_name}).to_string()),
                });
            }
        }
    }
...
fn extract_enum(node: &tree_sitter::Node, source: &[u8], exported: bool, ctx: &mut ExtractionCtx) {
    let name_node = match node.child_by_field_name("name") {
        Some(n) => n,
        None => return,
    };
    ...
    let enum_node = make_symbol_node(name, NodeSubtype::Enum, visibility, None, node, source, ctx);
        ...
        match child.kind() {
            ...
            "named_imports" => {
                ...
                for spec in child.children(&mut named_cursor) {
                    ...
                    let exported_name =
                        name_node.map(|n| n.utf8_text(source).unwrap_or("").to_string());
...
fn extract_calls_from_node(
    ...
    if node.kind() == "statement_block" {
        walk_block_sequentially(
            node,
            source,
            caller_node_uid,
            ctx,
            local_bindings,
            fn_bindings.as_deref_mut(),
        );
        return;
    }
    ...
    extract_calls_from_single_node(
        node,
        source,
        caller_node_uid,
        ctx,
        local_bindings,
        fn_bindings.as_deref(),
    );
...
fn collect_var_bindings(node: &tree_sitter::Node, source: &[u8], bindings: &mut BindingMap) {
    for i in 0..node.child_count() {
                ...
                if let Some(decl) = child.child(j) {
                    if decl.kind() == "variable_declarator" {
                        if let Some(name) = decl.child_by_field_name("name") {
                            bindings.insert(name.utf8_text(source).unwrap_or("").to_string(), None);
                        }
                    }
                }
            }
...
fn prescan_lexical_names(decl: &tree_sitter::Node, source: &[u8], bindings: &mut BindingMap) {
        ...
        if let Some(fn_node) = node.child_by_field_name("function") {
            if let Some(raw_name) = get_call_target_name(&fn_node, source) {
                if !is_builtin_call(&raw_name) {
                    let callee_name =
                        resolve_receiver_type(&raw_name, ctx, local_bindings, fn_bindings);
        ...
        if let Some(ctor_node) = node.child_by_field_name("constructor") {
            ...
            if !class_name.is_empty() {
                ctx.edges.push(ExtractedEdge {
                    ...
                    extractor: EXTRACTOR_NAME.into(),
                    location: Some(location_from_node(node)),
                    metadata_json: Some(serde_json::json!({"className": class_name}).to_string()),
                });
            }
        }
    }
...
fn get_call_target_name(fn_node: &tree_sitter::Node, source: &[u8]) -> Option<String> {
    match fn_node.kind() {
        "identifier" => Some(fn_node.utf8_text(source).unwrap_or("").to_string()),
        "member_expression" => {
...
fn classify_arg0_payload(call_node: &tree_sitter::Node, source: &[u8]) -> Option<CallArgPayload> {
...
mod tests {
    ...
    fn relative_import_produces_edge_and_binding() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, r#"import { Foo } from "./types";"#, "src/service.ts");
    ...
    fn namespace_import() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, r#"import * as utils from "./utils";"#, "src/index.ts");
    ...
    fn import_strips_js_extension() {
        ...
        let meta: serde_json::Value =
            serde_json::from_str(result.edges[0].metadata_json.as_ref().unwrap()).unwrap();
        assert_eq!(meta["resolvedPath"], "src/types");
    }
    ...
    fn side_effect_import_produces_no_binding() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, r#"import "./polyfill";"#, "src/index.ts");
    ...
    fn reexport_produces_imports_edge() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, r#"export { Foo } from "./types";"#, "src/index.ts");
    ...
    fn reexport_non_relative_no_edge() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, r#"export { useState } from "react";"#, "src/index.ts");
    ...
    fn exported_function_has_export_visibility() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "export function serve() {}", "src/server.ts");
    ...
    fn const_produces_constant_node() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "const MAX_SIZE = 1000;", "src/config.ts");
    ...
    fn let_produces_variable_node() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "let counter = 0;", "src/state.ts");
    ...
    fn class_produces_class_node_and_members() {
        ...
        assert_eq!(
            class.stable_key,
            "r1:src/service.ts#UserService:SYMBOL:CLASS"
        );
        ...
        assert_eq!(
            ctor.parent_node_uid.as_deref(),
            Some(class.node_uid.as_str())
        );
        ...
        assert_eq!(
            method.parent_node_uid.as_deref(),
            Some(class.node_uid.as_str())
        );
    ...
    fn exported_class() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "export class Service {}", "src/service.ts");
    ...
    fn getter_setter_subtypes() {
        ...
        let getter = result
            .nodes
            .iter()
            .find(|n| n.name == "name" && n.subtype == Some(NodeSubtype::Getter));
        assert!(getter.is_some(), "should find getter");

        let setter = result
            .nodes
            .iter()
            .find(|n| n.name == "name" && n.subtype == Some(NodeSubtype::Setter));
    ...
    fn interface_with_members() {
        ...
        let iface = result
            .nodes
            .iter()
            .find(|n| n.name == "StoragePort")
            .unwrap();
        assert_eq!(
            iface.stable_key,
            "r1:src/ports.ts#StoragePort:SYMBOL:INTERFACE"
        );
        ...
        assert_eq!(
            method.qualified_name.as_deref(),
            Some("StoragePort.getUser")
        );
        assert_eq!(
            method.parent_node_uid.as_deref(),
            Some(iface.node_uid.as_str())
        );
        assert_eq!(method.visibility, Some(Visibility::Public));
    ...
    fn interface_overloaded_methods_deduplicated() {
        ...
        assert_eq!(
            methods.len(),
            1,
            "overloads should be deduplicated to one node"
        );
    }
    ...
    fn type_alias_extraction() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "export type UserId = string;", "src/types.ts");
    ...
    fn enum_extraction() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "enum Status { Active, Inactive }", "src/enums.ts");
    ...
    fn function_call_produces_calls_edge() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "function main() { doStuff(); }", "src/main.ts");
    ...
    fn top_level_expression_call() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "app.listen(3000);", "src/index.ts");
    ...
    fn const_initializer_call() {
        let mut ext = TsExtractor::new();
        ext.initialize().unwrap();
        let result = extract_ok(&ext, "const result = compute(42);", "src/init.ts");
    ...
    fn block_local_const_binding_resolves_subsequent_call() {
        ...
        assert!(
            find_call.is_some(),
            "should find a find call, got: {:?}",
            calls.iter().map(|c| &c.target_key).collect::<Vec<_>>()
        );
...
"#,
        ...
        assert_eq!(
            component_nodes.len(),
            3,
            "should extract all three Component functions"
        );

        // Collect stable_keys and verify they are unique.
        let stable_keys: Vec<&str> = component_nodes
            .iter()
        ...
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#Component:SYMBOL:FUNCTION")),
            "first occurrence should have no :dup suffix"
        );
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#Component:SYMBOL:FUNCTION:dup2")),
            "second occurrence should have :dup2 suffix"
        );
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#Component:SYMBOL:FUNCTION:dup3")),
...
"#,
        ...
        assert_eq!(
            container_nodes.len(),
            2,
            "should extract both container constants"
        );

        let stable_keys: Vec<&str> = container_nodes
            .iter()
            .map(|n| n.stable_key.as_str())
            .collect();
        let unique_keys: std::collections::HashSet<&str> = stable_keys.iter().cloned().collect();
        ...
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#container:SYMBOL:CONSTANT")),
            "first occurrence should have no :dup suffix"
        );
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#container:SYMBOL:CONSTANT:dup2")),
            "second occurrence should have :dup2 suffix"
        );
    }
...
"#,
        ...
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#Thing:SYMBOL:FUNCTION")),
            "function Thing should have FUNCTION subtype"
        );
        assert!(
            stable_keys
                .iter()
                .any(|k| k.ends_with("#Thing:SYMBOL:CONSTANT")),
