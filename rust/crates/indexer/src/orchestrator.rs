...
pub fn index_repo<
    S: IndexerStoragePort
        + crate::storage_port::ProtoSchemaStorePort
        + crate::storage_port::GeneratedCodeMappingStorePort
        + crate::storage_port::GeneratedCodeMappingReadPort
        + crate::storage_port::GrpcImplHintReadPort
        + crate::storage_port::GrpcImplHintStorePort
        + crate::storage_port::GrpcRegistrationProofPort
        + crate::storage_port::GrpcClientHintReadPort
        + crate::storage_port::GrpcClientHintStorePort
        + crate::storage_port::GrpcLinkReadPort
        + crate::storage_port::GrpcLinkStorePort,
>(
    storage: &mut S,
    ...
    let empty_resource_keys: HashMap<String, crate::storage_port::CopiedResourceNodeKey> =
        HashMap::new();
    match run_pipeline(
        storage,
        extractors,
...
fn run_pipeline<S: IndexerStoragePort>(
    ...
    let mut emit = |phase: crate::types::IndexPhase,
                    current: u64,
                    total: u64,
                    file: Option<String>|
     -> Result<(), IndexError<S::StorageError>> {
        if let Some(ref mut cb) = progress {
            match cb(&crate::types::IndexProgressEvent {
                phase,
                current,
                total,
                file,
    ...
    for (file_idx, file) in files.iter().enumerate() {
        ...
        emit(
            crate::types::IndexPhase::Extracting,
            file_idx as u64 + 1,
            total_files,
        ...
        // Extract.
        let result =
            match extractor.extract(&file.content, &file.rel_path, &file_uid, repo_uid, snap_uid) {
                Ok(r) => r,
                Err(_e) => {
    ...
    if let Some(h) = hook {
        ...
        let node_to_file: std::collections::HashMap<&str, &str> = all_nodes
            .iter()
            .filter_map(|n| n.file_uid.as_deref().map(|f| (n.node_uid.as_str(), f)))
            .collect();
        ...
        if !result.still_unresolved.is_empty() {
            ...
            for ue in &result.still_unresolved {
                ...
                *unresolved_breakdown.entry(cat_key).or_insert(0) += 1;
                ...
                let file_signals = build_file_signals_for_edge(&ue.source_file_uid, &all_signals);
...
>(
    ...
    let config_paths: std::collections::BTreeSet<&str> =
        config_files.iter().map(|cf| cf.rel_path.as_str()).collect();
    plan.files_to_extract
        .retain(|p| !config_paths.contains(p.as_str()));
    plan.files_to_copy
        .retain(|p| !config_paths.contains(p.as_str()));
    ...
    let copied_tracked: Vec<TrackedFile> = plan
        ...
        .collect();
    storage
        .upsert_files(&copied_tracked)
        .map_err(IndexError::Storage)?;

    // Filter current_files to only those needing extraction.
    let extract_set: std::collections::HashSet<&str> =
        plan.files_to_extract.iter().map(|s| s.as_str()).collect();
...
mod tests {
    ...
    impl SnapshotLifecyclePort for MockStorage {
        ...
        fn update_snapshot_status(
            ...
            if let Some(s) = self
                .snapshots
                .iter_mut()
                .find(|s| s.snapshot_uid == input.snapshot_uid)
            {
    ...
    impl FileCatalogPort for MockStorage {
        ...
        fn get_stale_files(&self, _: &str) -> Result<Vec<TrackedFile>, String> {
            Ok(vec![])
        }
    ...
    impl NodeStorePort for MockStorage {
        ...
        fn delete_nodes_by_file(&mut self, _: &str, _: &str) -> Result<(), String> {
            Ok(())
        }
    }
    ...
    impl EdgeStorePort for MockStorage {
        ...
        fn query_extraction_edges_batch(
            ...
            limit: usize,
            after: Option<&str>,
        ) -> Result<Vec<ExtractionEdgeRow>, String> {
            let mut filtered: Vec<&ExtractionEdgeRow> = match after {
                Some(cursor) => self
                    .extraction_edges
                    .iter()
                    .filter(|e| e.edge_uid.as_str() > cursor)
                    .collect(),
    ...
    fn extractor_init_failure_returns_typed_error() {
        ...
        impl ExtractorPort for FailingExtractor {
            ...
            fn initialize(&mut self) -> Result<(), ExtractorError> {
                Err(ExtractorError {
                    message: "grammar not found".into(),
                })
            }
            fn extract(
                &self,
    ...
    fn storage_failure_transitions_snapshot_to_failed() {
        ...
        impl SnapshotLifecyclePort for FailOnInsertNodes {
            type Error = String;
            fn create_snapshot(&mut self, i: &CreateSnapshotInput) -> Result<Snapshot, String> {
                self.inner.create_snapshot(i)
            }
            fn get_snapshot(&self, uid: &str) -> Result<Option<Snapshot>, String> {
                self.inner.get_snapshot(uid)
            }
            fn get_latest_snapshot(&self, r: &str) -> Result<Option<Snapshot>, String> {
                self.inner.get_latest_snapshot(r)
            }
            fn update_snapshot_status(
                &mut self,
                i: &UpdateSnapshotStatusInput,
            ) -> Result<(), String> {
                self.inner.update_snapshot_status(i)
            }
            ...
            fn update_snapshot_extraction_diagnostics(
                ...
                self.inner.update_snapshot_extraction_diagnostics(s, d)
            }
        }
        impl FileCatalogPort for FailOnInsertNodes {
            type Error = String;
            fn upsert_files(&mut self, f: &[TrackedFile]) -> Result<(), String> {
                self.inner.upsert_files(f)
            }
            fn upsert_file_versions(&mut self, v: &[FileVersion]) -> Result<(), String> {
                self.inner.upsert_file_versions(v)
            }
            fn get_files_by_repo(&self, r: &str) -> Result<Vec<TrackedFile>, String> {
                self.inner.get_files_by_repo(r)
            }
            fn get_stale_files(&self, s: &str) -> Result<Vec<TrackedFile>, String> {
            ...
            fn query_file_version_hashes(
                ...
                self.inner.query_file_version_hashes(s)
            }
        }
        impl NodeStorePort for FailOnInsertNodes {
            ...
            fn query_all_nodes(&self, s: &str) -> Result<Vec<ExtractedNode>, String> {
                self.inner.query_all_nodes(s)
            }
            fn query_resolver_nodes(
                &self,
                s: &str,
            ) -> Result<Vec<crate::resolver::ResolverNode>, String> {
                self.inner.query_resolver_nodes(s)
            }
            fn delete_nodes_by_file(&mut self, s: &str, f: &str) -> Result<(), String> {
                self.inner.delete_nodes_by_file(s, f)
            }
        }
        impl EdgeStorePort for FailOnInsertNodes {
            ...
            fn insert_resolved_edges(
                &mut self,
                e: &[crate::resolver::ResolvedEdge],
            ) -> Result<(), String> {
                self.inner.insert_resolved_edges(e)
            }
            fn insert_extraction_edges(&mut self, e: &[ExtractionEdgeRow]) -> Result<(), String> {
                self.inner.insert_extraction_edges(e)
            }
            fn query_extraction_edges_batch(
                &self,
                s: &str,
                ...
                self.inner.query_extraction_edges_batch(s, l, a)
            }
        ...
        impl UnresolvedEdgePort for FailOnInsertNodes {
            ...
            fn insert_unresolved_edges(
                &mut self,
                e: &[PersistedUnresolvedEdge],
            ) -> Result<(), String> {
                self.inner.insert_unresolved_edges(e)
            }
        }
        impl FileSignalPort for FailOnInsertNodes {
            type Error = String;
            fn insert_file_signals(&mut self, s: &[FileSignalRow]) -> Result<(), String> {
                self.inner.insert_file_signals(s)
            }
            fn query_file_signals_batch(
                &self,
                s: &str,
                ...
                self.inner.query_file_signals_batch(s, f)
            }
        }
        impl crate::storage_port::DeltaCopyPort for FailOnInsertNodes {
            ...
            fn copy_forward_unchanged_files(
                &mut self,
                _: &crate::storage_port::CopyForwardInput,
            ) -> Result<crate::storage_port::CopyForwardResult, String> {
                Ok(crate::storage_port::CopyForwardResult::default())
            }
        }
        impl crate::storage_port::ProtoSchemaStorePort for FailOnInsertNodes {
            ...
            fn insert_proto_schema(
                &mut self,
                i: &crate::storage_port::ProtoSchemaInput,
            ) -> Result<(), String> {
                self.inner.insert_proto_schema(i)
            }
            fn insert_proto_elements(
                &mut self,
                e: &[crate::storage_port::ProtoElementInput],
            ) -> Result<usize, String> {
                self.inner.insert_proto_elements(e)
            }
        }
        impl crate::storage_port::GeneratedCodeMappingStorePort for FailOnInsertNodes {
            ...
            fn insert_generated_code_mappings(
                &mut self,
                m: &[crate::storage_port::GeneratedCodeMappingInput],
            ) -> Result<usize, String> {
                self.inner.insert_generated_code_mappings(m)
            }
            fn delete_generated_code_mappings_for_snapshot(
                &mut self,
                s: &str,
            ) -> Result<(), String> {
                self.inner.delete_generated_code_mappings_for_snapshot(s)
            }
        }
        impl crate::storage_port::GeneratedCodeMappingReadPort for FailOnInsertNodes {
            ...
            fn query_contract_elements_with_options(
                ...
                self.inner.query_contract_elements_with_options(s)
            }
            fn query_java_symbols(
                &self,
                s: &str,
            ) -> Result<Vec<crate::java_code_mapper::JavaSymbol>, String> {
                self.inner.query_java_symbols(s)
            }
        }
        impl crate::storage_port::GrpcImplHintReadPort for FailOnInsertNodes {
            ...
            fn query_impl_base_extensions(
                ...
                self.inner.query_impl_base_extensions(s)
            }
            fn query_impl_base_mappings(
                &self,
                s: &str,
            ) -> Result<Vec<crate::grpc_impl_hint::ImplBaseMappingInput>, String> {
                self.inner.query_impl_base_mappings(s)
            }
        }
        impl crate::storage_port::GrpcImplHintStorePort for FailOnInsertNodes {
            ...
            fn insert_grpc_impl_surfaces(
                &mut self,
                surfaces: &[crate::storage_port::GrpcImplSurfaceInput],
            ) -> Result<usize, String> {
                self.inner.insert_grpc_impl_surfaces(surfaces)
            }
            fn insert_grpc_impl_contracts(
                &mut self,
                contracts: &[crate::storage_port::GrpcImplContractInput],
            ) -> Result<usize, String> {
                self.inner.insert_grpc_impl_contracts(contracts)
            }
        }
        impl crate::storage_port::GrpcRegistrationProofPort for FailOnInsertNodes {
            ...
            fn query_add_service_calls(
                ...
                self.inner.query_add_service_calls(s)
            }
            fn find_grpc_impl_surface_by_class(
                &self,
                s: &str,
                ...
                self.inner.find_grpc_impl_surface_by_class(s, c, f)
            }
            fn boost_grpc_impl_confidence(
                &mut self,
                s: &str,
                r: &crate::storage_port::RegistrationSiteInput,
            ) -> Result<bool, String> {
                self.inner.boost_grpc_impl_confidence(s, r)
            }
        }
        impl crate::storage_port::GrpcClientHintReadPort for FailOnInsertNodes {
            ...
            fn query_grpc_stub_creations(
                ...
                self.inner.query_grpc_stub_creations(s)
            }
            fn query_grpc_service_mappings(
                &self,
                s: &str,
            ) -> Result<Vec<crate::storage_port::GrpcServiceMappingInput>, String> {
                self.inner.query_grpc_service_mappings(s)
            }
        }
        impl crate::storage_port::GrpcClientHintStorePort for FailOnInsertNodes {
            ...
            fn insert_grpc_client_surfaces(
                &mut self,
                surfaces: &[crate::storage_port::GrpcClientSurfaceInput],
            ) -> Result<usize, String> {
                self.inner.insert_grpc_client_surfaces(surfaces)
            }
            fn insert_grpc_client_contracts(
                &mut self,
                contracts: &[crate::storage_port::GrpcClientContractInput],
            ) -> Result<usize, String> {
                self.inner.insert_grpc_client_contracts(contracts)
            }
        }
        impl crate::storage_port::GrpcLinkReadPort for FailOnInsertNodes {
            ...
            fn query_provider_surfaces_with_contracts(
                ...
                self.inner.query_provider_surfaces_with_contracts(s)
            }
            fn query_consumer_surfaces_with_contracts(
                &self,
                s: &str,
            ) -> Result<Vec<crate::storage_port::SurfaceWithContract>, String> {
                self.inner.query_consumer_surfaces_with_contracts(s)
            }
        }
        impl crate::storage_port::GrpcLinkStorePort for FailOnInsertNodes {
            ...
            fn insert_boundary_interaction_links(
                &mut self,
                links: &[crate::storage_port::BoundaryInteractionLinkInput],
            ) -> Result<usize, String> {
                self.inner.insert_boundary_interaction_links(links)
            }
        }
    ...
    fn progress_callback_abort_stops_extraction() {
        ...
        let phases = events.lock().unwrap();
        let extract_count = phases
            .iter()
            .filter(|p| **p == crate::types::IndexPhase::Extracting)
            .count();
        assert_eq!(
            extract_count, 2,
            "expected 2 extracting events before abort, got {}",
            extract_count
        );
    ...
    fn abort_before_snapshot_creation_prevents_db_mutation() {
        ...
        let mut progress_callback =
            |_evt: &crate::types::IndexProgressEvent| -> ControlFlow<()> { ControlFlow::Break(()) };
        ...
        assert!(
            storage.snapshots.is_empty(),
            "no snapshot should exist after pre-pipeline abort"
        );
    }
    ...
    fn refresh_creates_refresh_snapshot_with_copy_forward() {
        ...
        assert_eq!(refresh_snap.parent_snapshot_uid, Some(parent_snap_uid));
