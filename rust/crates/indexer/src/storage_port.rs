...
use crate::types::{EdgeType, ParseStatus, Resolution, SnapshotKind, SnapshotStatus};
...
pub trait SnapshotLifecyclePort {
    ...
    fn create_snapshot(&mut self, input: &CreateSnapshotInput) -> Result<Snapshot, Self::Error>;

    /// Look up a snapshot by UID. Returns `None` if not found.
    fn get_snapshot(&self, snapshot_uid: &str) -> Result<Option<Snapshot>, Self::Error>;

    /// Get the latest READY snapshot for a repo. Returns `None` if
    /// no ready snapshot exists.
    fn get_latest_snapshot(&self, repo_uid: &str) -> Result<Option<Snapshot>, Self::Error>;
...
pub trait FileCatalogPort {
    ...
    fn upsert_files(&mut self, files: &[TrackedFile]) -> Result<(), Self::Error>;

    /// Insert or update file versions for a snapshot.
    fn upsert_file_versions(&mut self, versions: &[FileVersion]) -> Result<(), Self::Error>;

    /// Get all non-excluded tracked files for a repo.
    fn get_files_by_repo(&self, repo_uid: &str) -> Result<Vec<TrackedFile>, Self::Error>;

    /// Get files with stale parse status in a snapshot.
    fn get_stale_files(&self, snapshot_uid: &str) -> Result<Vec<TrackedFile>, Self::Error>;
...
pub trait NodeStorePort {
    ...
    fn insert_nodes(&mut self, nodes: &[ExtractedNode]) -> Result<(), Self::Error>;

    /// Query all nodes in a snapshot (full GraphNode shape).
    fn query_all_nodes(&self, snapshot_uid: &str) -> Result<Vec<ExtractedNode>, Self::Error>;

    /// Query slim resolver nodes for building the ResolverIndex.
    /// Returns all nodes in the snapshot with only the fields
    /// needed for resolution.
    fn query_resolver_nodes(&self, snapshot_uid: &str) -> Result<Vec<ResolverNode>, Self::Error>;
...
pub trait EdgeStorePort {
    ...
    fn insert_resolved_edges(&mut self, edges: &[ResolvedEdge]) -> Result<(), Self::Error>;

    /// Insert a batch of extraction edges (durable unresolved
    /// edges with source_file_uid). Transaction-wrapped.
    fn insert_extraction_edges(&mut self, edges: &[ExtractionEdgeRow]) -> Result<(), Self::Error>;
...
pub trait FileSignalPort {
    ...
    fn insert_file_signals(&mut self, signals: &[FileSignalRow]) -> Result<(), Self::Error>;
...
pub trait ProtoSchemaStorePort {
    ...
    fn insert_proto_schema(&mut self, input: &ProtoSchemaInput) -> Result<(), Self::Error>;
