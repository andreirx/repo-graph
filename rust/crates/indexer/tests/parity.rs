...
fn run_fixture(fixture: &Fixture) -> Result<Value, String> {
    ...
    match fn_name.as_str() {
        ...
        "categorize_unresolved_edge" => {
            ...
            let metadata_json: Option<String> = fixture.input.get("metadataJson").and_then(|v| {
                if v.is_null() {
                    None
                } else {
...
fn parity_against_shared_indexer_fixture_corpus() {
    ...
    assert!(
        !fixtures.is_empty(),
        "no fixtures found in indexer-parity-fixtures/"
    );
    ...
    if !generated.is_empty() {
        panic!(
            "Generated {} expected.json. Re-run to verify.",
            generated.len()
        );
    }
}
