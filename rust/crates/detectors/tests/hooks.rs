...
use repo_graph_detectors::types::{
    DetectorFamily, DetectorLanguage, Emitted, EnvAccessKind, EnvAccessPattern, EnvDetectorRecord,
    EnvHookContext, EnvHookResult, FsDetectorRecord, FsHookContext, FsHookResult, MutationKind,
    MutationPattern,
};
...
/// the line.
fn run_env_hook(hook_name: &'static str, regex_pattern: &str, line: &str) -> EnvHookResult {
...
fn run_fs_hook(hook_name: &'static str, regex_pattern: &str, line: &str) -> FsHookResult {
    let registry = create_default_hook_registry();
    let entry = registry
        .fs
        .get(hook_name)
...
fn dispatch_python_remove_dispatches_os_remove() {
    ...
    let records = unwrap_fs_records(result);
    assert_eq!(
        records[0].mutation_pattern,
        Some(MutationPattern::PyOsRemove)
    );
}
...
fn dispatch_python_remove_dispatches_os_unlink() {
    ...
    assert_eq!(
        records[0].mutation_pattern,
        Some(MutationPattern::PyOsUnlink)
    );
}
...
fn dispatch_python_remove_dispatches_shutil_rmtree() {
    ...
    assert_eq!(
        records[0].mutation_pattern,
        Some(MutationPattern::PyShutilRmtree)
    );
}
...
fn dispatch_python_mkdir_dispatches_mkdir() {
    ...
    assert_eq!(
        records[0].mutation_pattern,
        Some(MutationPattern::PyOsMkdir)
    );
}
...
fn dispatch_python_mkdir_dispatches_makedirs() {
    ...
    assert_eq!(
        records[0].mutation_pattern,
        Some(MutationPattern::PyOsMakedirs)
    );
}
