...
fn env_ts_process_env_dot_with_fallback_is_optional_with_default() {
    let out = detect_env_accesses(r#"const port = process.env.PORT || "3000";"#, "src/foo.ts");
    assert_eq!(out.len(), 1);
    let r = &out[0];
...
fn env_ts_bracket_access_detected() {
    let out = detect_env_accesses(r#"const port = process.env["PORT"];"#, "src/foo.ts");
    assert_eq!(out.len(), 1);
    assert_eq!(out[0].var_name, "PORT");
    assert_eq!(out[0].access_pattern, EnvAccessPattern::ProcessEnvBracket);
}

#[test]
fn env_python_os_environ_detected() {
    let out = detect_env_accesses(r#"port = os.environ["PORT"]"#, "src/app.py");
    assert_eq!(out.len(), 1);
    assert_eq!(out[0].var_name, "PORT");
...
fn env_python_os_getenv_with_default_is_optional() {
    let out = detect_env_accesses(r#"port = os.getenv("PORT", "3000")"#, "src/app.py");
    assert_eq!(out.len(), 1);
...
fn env_java_system_getenv_detected() {
    let out = detect_env_accesses(r#"String port = System.getenv("PORT");"#, "src/Main.java");
    assert_eq!(out.len(), 1);
    assert_eq!(out[0].var_name, "PORT");
...
fn env_c_getenv_detected() {
    let out = detect_env_accesses(r#"char *port = getenv("PORT");"#, "src/main.c");
    assert_eq!(out.len(), 1);
    assert_eq!(out[0].var_name, "PORT");
...
fn fs_ts_writefile_literal_detection_produces_expected_shape() {
    let out = detect_fs_mutations(r#"fs.writeFile("out.txt", data, cb);"#, "src/foo.ts");
    assert_eq!(out.len(), 1);
    let r = &out[0];
...
fn fs_ts_rename_captures_destination() {
    let out = detect_fs_mutations(r#"fs.rename("a.txt", "b.txt", cb);"#, "src/foo.ts");
    assert_eq!(out.len(), 1);
...
fn fs_rust_fs_write_literal_wins_over_catchall_via_position_dedup() {
    let out = detect_fs_mutations(r#"std::fs::write("output.txt", data)?;"#, "src/main.rs");
...
fn fs_c_fopen_dispatches_append_vs_write() {
    let write_out = detect_fs_mutations(r#"FILE *f = fopen("data.bin", "wb");"#, "src/main.c");
    ...
    let append_out = detect_fs_mutations(r#"FILE *f = fopen("audit.log", "a");"#, "src/main.c");
    assert_eq!(append_out.len(), 1);
    assert_eq!(append_out[0].mutation_kind, MutationKind::AppendFile);
    assert_eq!(
        append_out[0].mutation_pattern,
...
fn fs_python_open_dispatches_write_vs_append() {
    ...
    let append_out = detect_fs_mutations(r#"open("audit.log", "a")"#, "src/app.py");
    assert_eq!(append_out.len(), 1);
    assert_eq!(append_out[0].mutation_kind, MutationKind::AppendFile);
    assert_eq!(
        append_out[0].mutation_pattern,
...
fn env_inside_block_comment_is_not_detected() {
    let out = detect_env_accesses("/* process.env.PHANTOM */", "src/foo.ts");
    assert!(out.is_empty());
}
...
fn python_env_inside_hash_comment_is_not_detected() {
    let out = detect_env_accesses("# os.environ[\"PHANTOM\"] is commented\n", "src/app.py");
    assert!(out.is_empty());
}
