...
use repo_graph_detectors::types::{
    DetectorFamily, DetectorRecord, EnvHook, EnvHookEntry, EnvHookResult, EnvSuppliedField, FsHook,
    FsHookEntry, FsHookResult, FsSuppliedField, HookRegistry,
};
...
fn assert_validation_err_contains(
    ...
    match res {
        Ok(v) => {
            panic!("expected validation error containing {expected_substring:?}, got Ok({v:?})")
        }
...
fn assert_toml_parse_err_contains(
    ...
    match res {
        Ok(v) => {
            panic!("expected TOML parse error containing {expected_substring:?}, got Ok({v:?})")
        }
        Err(DetectorLoadError::TomlParse(msg)) => {
            assert!(
                msg.contains(expected_substring),
    ...
    let registry = make_registry(
        &[EnvHookSpec {
            ...
            supplies: &[
                ...
                EnvSuppliedField::AccessPattern,
            ],
        }],
        &[],
    );
...
"#;
    ...
    match &graph.all_records[0] {
        DetectorRecord::Fs(f) => {
            assert_eq!(
                f.position_dedup_group.as_deref(),
                Some("rust_fs_write_callsite")
            );
        }
        _ => panic!("expected fs record"),
    }
...
"#;
    let res = load_detector_graph(toml, &empty_registry());
    assert_validation_err_contains(res, "unknown field \"position_dedup_group\" for env family");
}
