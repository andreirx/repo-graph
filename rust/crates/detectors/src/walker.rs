...
use crate::types::{
    DetectedEnvDependency, DetectedFsMutation, DetectorFamily, DetectorLanguage, DetectorRecord,
    Emitted, EnvDetectorRecord, EnvHookContext, EnvHookResult, FsDetectorRecord, FsHookContext,
    FsHookResult, HookRegistry, LoadedDetectorGraph,
};
    ...
    for (line_idx, line) in content.split('\n').enumerate() {
        ...
        for record in records {
            ...
            for captures in regex.captures_iter(line) {
                ...
                let emissions =
                    emit_fs_from_match(fs_record, captures, line, line_number, file_path, registry);
                for e in emissions {
                    results.push(e);
                }
