...
use crate::types::{
    env_must_be_supplied, fs_must_be_supplied, DetectorFamily, DetectorLanguage, DetectorRecord,
    EnvAccessKind, EnvAccessPattern, EnvDetectorRecord, EnvSuppliedField, FsDetectorRecord,
    FsSuppliedField, HookRegistry, LoadedDetectorGraph, MutationKind, MutationPattern,
...
pub fn load_detector_graph(
    ...
    for (i, raw) in document.detector.into_iter().enumerate() {
        ...
        let key = match &record {
            DetectorRecord::Env(r) => (r.language, DetectorFamily::Env, r.pattern_id.clone()),
            DetectorRecord::Fs(r) => (r.language, DetectorFamily::Fs, r.pattern_id.clone()),
        };
    ...
    let mut by_family_language: HashMap<(DetectorFamily, DetectorLanguage), Vec<DetectorRecord>> =
        HashMap::new();
    for r in &records {
        let key = r.family_language();
        by_family_language.entry(key).or_default().push(r.clone());
    }
...
fn validate_record(
    ...
    let confidence = raw.confidence.ok_or_else(|| {
        DetectorLoadError::Validation(format!("{ctx}: missing required field \"confidence\""))
    })?;
...
fn validate_env_record(
    ...
    for required in env_must_be_supplied() {
        if !static_supplied.contains(&required) && !hook_supplies.contains(&required) {
...
fn compile_regex(source: &str, flags: &str, ctx: &str) -> Result<Regex, DetectorLoadError> {
    ...
    let compiled = builder
        .build()
        .map_err(|e| DetectorLoadError::Validation(format!("{ctx}: regex compile error: {e}")))?;
...
fn parse_access_kind(s: &str, ctx: &str) -> Result<EnvAccessKind, DetectorLoadError> {
    match s {
...
fn parse_access_pattern(s: &str, ctx: &str) -> Result<EnvAccessPattern, DetectorLoadError> {
    match s {
...
fn parse_mutation_kind(s: &str, ctx: &str) -> Result<MutationKind, DetectorLoadError> {
    match s {
...
fn parse_mutation_pattern(s: &str, ctx: &str) -> Result<MutationPattern, DetectorLoadError> {
    match s {
