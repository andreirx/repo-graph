...
fn discover_fixtures() -> Vec<Fixture> {
    ...
    for entry in WalkDir::new(&root)
        ...
        .sort_by_file_name()
    {
        let entry = entry.unwrap_or_else(|e| panic!("failed to walk {}: {e}", root.display()));
...
fn run_fixture(fixture: &Fixture) -> Result<Value, String> {
    ...
    match fn_name {
        ...
        "detect_missing_entrypoint_declarations" => {
            dispatch_detect_missing_entrypoints(&fixture.input)
        }
...
fn dispatch_detect_registry_pattern(input: &Value) -> Result<Value, String> {
    let by_ancestor: Vec<(String, usize)> = get_field(
        input,
        "pathPrefixCyclesByAncestor",
        "detect_registry_pattern",
    )?;
...
fn dispatch_compute_call_graph(input: &Value) -> Result<Value, String> {
    ...
    let unresolved: u64 = get_field(input, "unresolvedCallsInternalLike", "compute_call_graph")?;
...
fn dispatch_compute_import_graph(input: &Value) -> Result<Value, String> {
    ...
    let registry: bool = get_field(input, "registryPatternSuspicion", "compute_import_graph")?;
    let unresolved: u64 = get_field(input, "unresolvedImportsCount", "compute_import_graph")?;
...
fn dispatch_compute_dead_code(input: &Value) -> Result<Value, String> {
    let missing_ep: bool = get_field(input, "missingEntrypointDeclarations", "compute_dead_code")?;
    let registry: bool = get_field(input, "registryPatternSuspicion", "compute_dead_code")?;
    let framework: bool = get_field(input, "frameworkHeavySuspicion", "compute_dead_code")?;
    let call_level: ReliabilityLevel = get_field(input, "callGraphLevel", "compute_dead_code")?;
...
fn dispatch_compute_change_impact(input: &Value) -> Result<Value, String> {
    let alias: bool = get_field(input, "aliasResolutionSuspicion", "compute_change_impact")?;
    let registry: bool = get_field(input, "registryPatternSuspicion", "compute_change_impact")?;
...
impl TrustStorageRead for FixtureMockStorage {
    ...
    fn compute_module_stats(&self, _snapshot_uid: &str) -> Result<Vec<TrustModuleStats>, String> {
...
fn dispatch_assemble_trust_report(input: &Value) -> Result<Value, String> {
    ...
    // parsing path.
    let toolchain_json: Option<String> = input.get("toolchain").and_then(|v| {
        if v.is_null() {
            None
        } else {
    ...
    let diagnostics_json: Option<String> = input.get("diagnostics").and_then(|v| {
        if v.is_null() {
            None
        } else {
            Some(serde_json::to_string(v).unwrap())
