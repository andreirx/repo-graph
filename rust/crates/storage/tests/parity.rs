...
use repo_graph_storage::types::{
    CreateSnapshotInput, FileVersion, GraphEdge, GraphNode, Repo, RepoRef, TrackedFile,
        ...
        Value::String(s) => {
            if let Some(rest) = s.strip_prefix('@') {
                ...
                binding
                    .get(field)
                    .cloned()
                    .ok_or_else(|| format!("binding '{}' has no field '{}'", binding_name, field))
            } else {
...
fn dispatch_op(
    ...
    let resolved =
        resolve_refs(raw_op, &state.bindings).map_err(|e| format!("{}: {}", op_kind, e))?;
    ...
    match op_kind.as_str() {
        ...
        "createSnapshot" => {
            ...
            state
                .substitutions
                .push((snap.snapshot_uid.clone(), format!("<SNAP:{}>", name)));
            state
                .substitutions
                .push((snap.created_at.clone(), format!("<TS:{}:createdAt>", name)));
        ...
        "getLatestSnapshot" => {
            let repo_uid: String = from_json("getLatestSnapshot.repoUid", get_field("repoUid")?)?;
            let result = storage
        ...
        "updateSnapshotCounts" => {
            let uid: String = from_json(
                "updateSnapshotCounts.snapshotUid",
        ...
        "upsertFileVersions" => {
            let versions: Vec<FileVersion> = from_json(
                "upsertFileVersions.fileVersions",
                get_field("fileVersions")?,
            )?;
            storage
        ...
        "getFilesByRepo" => {
            let repo_uid: String = from_json("getFilesByRepo.repoUid", get_field("repoUid")?)?;
            let files = storage
        ...
        "deleteNodesByFile" => {
            ...
            let file_uid: String = from_json("deleteNodesByFile.fileUid", get_field("fileUid")?)?;
            storage
...
fn from_json<T: for<'de> serde::Deserialize<'de>>(label: &str, value: Value) -> Result<T, String> {
...
fn run_fixture(fixture: &Fixture) -> Result<(), String> {
    let mut storage =
        StorageConnection::open_in_memory().map_err(|e| format!("open_in_memory: {}", e))?;
    ...
    for (i, op) in ops.iter().enumerate() {
        dispatch_op(&mut storage, &mut state, op).map_err(|e| format!("op[{}]: {}", i, e))?;
    }
