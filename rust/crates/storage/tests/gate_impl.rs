...
fn insert_waiver_declaration(
    ...
    storage
        .insert_declaration(&DeclarationInsert {
            identity_key: waiver_identity_key(repo_uid, req_id, requirement_version, obligation_id),
            repo_uid: repo_uid.to_string(),
            target_stable_key: target_key,
...
fn requirements_projection_maps_to_gate_dtos() {
    ...
    let reqs =
        <StorageConnection as GateStorageRead>::get_active_requirements(&storage, "r1").unwrap();
...
fn boundary_declarations_projection_maps_module_forbids_reason() {
    ...
    let boundaries =
        <StorageConnection as GateStorageRead>::get_boundary_declarations(&storage, "r1").unwrap();
...
fn find_boundary_imports_returns_gate_import_edges() {
    ...
    let imports = <StorageConnection as GateStorageRead>::find_boundary_imports(
        &storage,
        &snapshot_uid,
        "src/core",
        "src/adapters",
...
fn coverage_measurements_projection_maps_to_gate_dtos() {
    ...
    let measurements =
        <StorageConnection as GateStorageRead>::get_coverage_measurements(&storage, &snapshot_uid)
            .unwrap();
...
fn hotspot_inferences_projection_maps_to_gate_dtos() {
    ...
    let inferences =
        <StorageConnection as GateStorageRead>::get_hotspot_inferences(&storage, &snapshot_uid)
            .unwrap();
...
fn storage_error_maps_to_gate_storage_error() {
    ...
    let result = <StorageConnection as GateStorageRead>::get_active_requirements(&storage, "r1");
...
fn quality_assessment_facts_returns_one_fact_per_active_policy() {
    ...
    assert_eq!(
        facts[0].policy_kind,
        repo_graph_gate::GateQualityPolicyKind::AbsoluteMax
    );
    assert_eq!(
        facts[0].severity,
        repo_graph_gate::GateQualityPolicySeverity::Fail
    );
    assert_eq!(
        facts[0].assessment_state,
        repo_graph_gate::GateAssessmentState::Missing
    );
    ...
    assert_eq!(
        facts[1].policy_kind,
        repo_graph_gate::GateQualityPolicyKind::NoNew
    );
    assert_eq!(
        facts[1].severity,
        repo_graph_gate::GateQualityPolicySeverity::Advisory
    );
    assert_eq!(
        facts[1].assessment_state,
        repo_graph_gate::GateAssessmentState::Missing
    );
}
...
fn quality_assessment_facts_joins_with_assessment_rows() {
    ...
    assert_eq!(
        fact.assessment_state,
        repo_graph_gate::GateAssessmentState::Present
    );
    assert_eq!(
...
fn quality_assessment_facts_comparative_policy_shows_baseline() {
    ...
    let policy_uid =
        insert_quality_policy_declaration(&storage, "r1", "QP-002", 1, "no_worsened", "fail");
    ...
    assert_eq!(
        fact.policy_kind,
        repo_graph_gate::GateQualityPolicyKind::NoWorsened
    );
    assert_eq!(
        fact.assessment_state,
        repo_graph_gate::GateAssessmentState::Present
    ...
    assert_eq!(
        fact.baseline_snapshot_uid.as_deref(),
        Some(baseline_uid.as_str())
    );
}
