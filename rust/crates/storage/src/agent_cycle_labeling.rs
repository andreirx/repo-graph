...
pub(crate) fn label_module_cycles(
    ...
    let files_ref: Vec<(&str, bool)> = files.iter().map(|(p, t)| (p.as_str(), *t)).collect();
...
fn repo_tracked_files(
    conn: &StorageConnection,
    snapshot_uid: &str,
) -> Result<Vec<(String, bool)>, AgentStorageError> {
    let repo_uid = conn
    ...
    let files = conn
        ...
        .map_err(map_err("find_module_cycles"))?;
    Ok(files.into_iter().map(|f| (f.path, f.is_test)).collect())
}
