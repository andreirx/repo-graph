...
//!    migration runner, applies all 20 migrations. Returns
...
/// underlying SQLite connection AND runs all 20 migrations
...
mod tests {
	...
	fn open_in_memory_returns_a_fully_initialized_connection() {
		...
		// Verify all 20 migrations have been applied by checking
		...
		assert_eq!(
			count, 20,
			"open_in_memory must run all 20 migrations before returning"
		);
	}
	...
	fn open_in_memory_schema_includes_all_expected_tables() {
		...
		let table_count: i64 = storage
			...
			.unwrap();
		// 20 migrations introduce 34 tables (per the
	...
	fn re_opening_an_existing_file_backed_database_is_idempotent() {
		...
		// Verify schema_migrations still has exactly 20 rows
