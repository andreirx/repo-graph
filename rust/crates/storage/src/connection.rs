...
//!    migration runner, applies all 19 migrations. Returns
...
/// underlying SQLite connection AND runs all 19 migrations
...
mod tests {
	...
	fn open_in_memory_returns_a_fully_initialized_connection() {
		...
		// Verify all 19 migrations have been applied by checking
		...
		assert_eq!(
			count, 19,
			"open_in_memory must run all 19 migrations before returning"
		);
	}
	...
	fn open_in_memory_schema_includes_all_expected_tables() {
		...
		let table_count: i64 = storage
			...
			.unwrap();
		// 19 migrations introduce 33 tables (per the
