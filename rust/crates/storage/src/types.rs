...
/// surfaces. The UID is generated by the storage layer.
///
/// Required fields match the `project_surfaces` table schema.
/// `stable_surface_key` is mandatory for Rust-produced surfaces
/// (framework detection always has deterministic identity).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
///
/// FD-SUPPORT-1: Links a project surface to its detection evidence
/// (source file, evidence kind, payload).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
