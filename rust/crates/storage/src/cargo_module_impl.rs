...
use repo_graph_indexer::cargo_manifest::{
    CargoModuleCandidateInput, CargoModuleEvidenceInput, CargoModuleStorePort, FileOwnershipInput,
};
...
mod tests {
    ...
    fn insert_and_read_cargo_module_candidates() {
        ...
        assert_eq!(
            ...
            Some("repo-graph-storage".to_string())
        );
    }
    ...
    fn insert_and_read_cargo_module_evidence() {
        ...
        let evidence = vec![CargoModuleEvidenceInput {
            ...
            confidence: 1.0,
            payload_json:
                r#"{"package_name":"core","crate_root":"crates/core","workspace_member":true}"#
                    .to_string(),
        }];
