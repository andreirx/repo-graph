...
impl StorageConnection {
    ...
    pub fn query_all_nodes(&self, snapshot_uid: &str) -> Result<Vec<GraphNode>, StorageError> {
        let conn = self.connection();
        let mut stmt = conn.prepare("SELECT * FROM nodes WHERE snapshot_uid = ?")?;
        let rows = stmt
...
mod tests {
    ...
    fn delete_nodes_by_file_deletes_nodes_and_incident_edges_in_one_transaction() {
        ...
        let edge = make_edge("e1", &snap_uid, "r1", &node_a.node_uid, &node_b.node_uid);
        ...
        assert_eq!(
            edge_count, 0,
            "incident edge must be deleted along with node"
        );
    }
}
