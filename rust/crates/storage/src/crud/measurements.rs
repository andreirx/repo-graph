...
mod tests {
    ...
    fn replace_measurements_by_kind_atomic_rollback() {
        ...
        let result =
            storage.replace_measurements_by_kind(&snap_uid, &["line_coverage"], &replacement);
        assert!(
            result.is_err(),
            "duplicate UIDs in batch must cause failure"
        );
        ...
        assert!(
            rows[0].value_json.contains("0.5"),
