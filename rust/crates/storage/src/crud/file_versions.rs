...
impl StorageConnection {
    ...
    pub fn upsert_file_versions(&mut self, versions: &[FileVersion]) -> Result<(), StorageError> {
    ...
    pub fn query_file_version_hashes(
        ...
        let mut stmt = conn
            .prepare("SELECT file_uid, content_hash FROM file_versions WHERE snapshot_uid = ?")?;
...
mod tests {
    ...
    fn setup_with_snapshot(storage: &mut StorageConnection) -> String {
        ...
        let snap = storage
            ...
            .unwrap();
        storage
            .upsert_files(&[make_file("r1", "src/a.ts"), make_file("r1", "src/b.ts")])
            .unwrap();
        snap.snapshot_uid
    }
    ...
    fn upsert_file_versions_then_query_hashes_returns_map() {
        ...
        assert_eq!(
            hashes.get("r1:src/a.ts").map(|s| s.as_str()),
            Some("hash-a")
        );
        assert_eq!(
            hashes.get("r1:src/b.ts").map(|s| s.as_str()),
            Some("hash-b")
        );
    }
