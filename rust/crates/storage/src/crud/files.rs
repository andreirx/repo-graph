...
impl StorageConnection {
    ...
    /// R2-E.
    pub fn get_files_by_repo(&self, repo_uid: &str) -> Result<Vec<TrackedFile>, StorageError> {
        let conn = self.connection();
        let mut stmt = conn
            .prepare("SELECT * FROM files WHERE repo_uid = ? AND is_excluded = 0 ORDER BY path")?;
        let rows = stmt
    ...
    pub fn get_stale_files(&self, snapshot_uid: &str) -> Result<Vec<TrackedFile>, StorageError> {
        let conn = self.connection();
...
mod tests {
    ...
    use crate::crud::test_helpers::{fresh_storage, make_file, make_file_version, make_repo};
    ...
    fn get_stale_files_returns_only_files_with_stale_parse_status() {
        ...
        let fv_parsed: FileVersion = make_file_version(&snap.snapshot_uid, &f_parsed.file_uid);
