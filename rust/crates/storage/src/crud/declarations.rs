...
impl StorageConnection {
    ...
    pub fn deactivate_declaration(&self, declaration_uid: &str) -> Result<usize, StorageError> {
...
pub fn discovered_module_boundary_identity_key(
    ...
    format!(
        "discovered_module:{}:{}:{}",
        repo_uid, source_path, target_path
    )
}
...
pub fn waiver_identity_key(
    ...
    format!(
        "{}:{}:{}:{}",
        repo_uid, req_id, requirement_version, obligation_id
    )
}
...
pub fn quality_policy_waiver_identity_key(
    ...
    format!(
        "{}:{}:{}:{}",
        repo_uid, policy_id, policy_version, target_stable_key
    )
}
...
mod tests {
    ...
    fn insert_declaration_cosmetic_value_change_is_idempotent() {
        ...
        assert!(
            !second.inserted,
            "cosmetic change must not create duplicate"
        );
    }
    ...
    fn deactivate_declaration_hides_from_active_queries() {
        ...
        let rows = storage
            .deactivate_declaration(&result.declaration_uid)
            .unwrap();
        assert_eq!(rows, 1);
    ...
    fn supersede_declaration_success() {
        ...
        assert_ne!(
            result.new_declaration_uid, old_result.declaration_uid,
            "new UID must differ"
        );
    ...
    fn supersede_declaration_old_inactive_is_error() {
        ...
        let old_result = storage.insert_declaration(&old_decl).unwrap();
        storage
            .deactivate_declaration(&old_result.declaration_uid)
            .unwrap();
    ...
    fn supersede_declaration_is_atomic_on_insert_failure() {
        ...
        assert_eq!(
            active.len(),
            1,
            "exactly one active boundary (the original)"
        );
