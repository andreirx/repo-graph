...
impl SnapshotLifecyclePort for StorageConnection {
    ...
    fn get_snapshot(&self, snapshot_uid: &str) -> Result<Option<ixp::Snapshot>, StorageError> {
        let snap = <StorageConnection>::get_snapshot(self, snapshot_uid)?;
        snap.map(from_storage_snapshot).transpose()
    }

    fn get_latest_snapshot(&self, repo_uid: &str) -> Result<Option<ixp::Snapshot>, StorageError> {
    ...
    fn update_snapshot_counts(&mut self, snapshot_uid: &str) -> Result<(), StorageError> {
...
impl FileCatalogPort for StorageConnection {
    ...
    fn upsert_files(&mut self, files: &[ixp::TrackedFile]) -> Result<(), StorageError> {
    ...
    fn upsert_file_versions(&mut self, versions: &[ixp::FileVersion]) -> Result<(), StorageError> {
    ...
    fn get_files_by_repo(&self, repo_uid: &str) -> Result<Vec<ixp::TrackedFile>, StorageError> {
        let files = <StorageConnection>::get_files_by_repo(self, repo_uid)?;
        Ok(files.into_iter().map(from_storage_tracked_file).collect())
    }

    fn get_stale_files(&self, snapshot_uid: &str) -> Result<Vec<ixp::TrackedFile>, StorageError> {
...
impl NodeStorePort for StorageConnection {
    ...
    fn insert_nodes(&mut self, nodes: &[ixp::ExtractedNode]) -> Result<(), StorageError> {
    ...
    fn query_all_nodes(&self, snapshot_uid: &str) -> Result<Vec<ixp::ExtractedNode>, StorageError> {
...
impl EdgeStorePort for StorageConnection {
    ...
    fn insert_resolved_edges(&mut self, edges: &[ixp::ResolvedEdge]) -> Result<(), StorageError> {
    ...
    fn insert_extraction_edges(&mut self, edges: &[ExtractionEdgeRow]) -> Result<(), StorageError> {
    ...
    fn query_extraction_edges_batch(
        ...
        let (sql, params): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = match after_edge_uid {
            Some(cursor) => (
            ...
            None => (
                "SELECT edge_uid, snapshot_uid, repo_uid, source_node_uid, \
					 ...
					 ORDER BY edge_uid ASC LIMIT ?"
                    .into(),
                vec![Box::new(snapshot_uid.to_string()), Box::new(limit as i64)],
            ),
        };
...
impl FileSignalPort for StorageConnection {
    ...
    fn insert_file_signals(&mut self, signals: &[FileSignalRow]) -> Result<(), StorageError> {
        let tx = self.connection_mut().transaction()?;
        {
            let mut stmt = tx.prepare(
...
mod tests {
    ...
    fn get_snapshot_via_trait_returns_none_for_missing() {
        let storage = setup();
        let result = SnapshotLifecyclePort::get_snapshot(&storage, "nonexistent").unwrap();
    ...
    fn update_snapshot_status_via_trait() {
        ...
        let updated = SnapshotLifecyclePort::get_snapshot(&storage, &snap.snapshot_uid)
            .unwrap()
            .unwrap();
    ...
    fn update_extraction_diagnostics_via_trait() {
        ...
        let diag =
            TrustStorageRead::get_snapshot_extraction_diagnostics(&storage, &snap.snapshot_uid)
                .unwrap();
        assert_eq!(diag, Some(r#"{"diagnostics_version":1}"#.to_string()));
    }
    ...
    fn query_file_version_hashes_returns_btreemap() {
        ...
        let hashes =
            FileCatalogPort::query_file_version_hashes(&storage, &snap.snapshot_uid).unwrap();
    ...
    fn get_snapshot_errors_on_bad_kind_value() {
        ...
        let result = SnapshotLifecyclePort::get_snapshot(&storage, &snap.snapshot_uid);
        assert!(
    ...
    fn insert_and_query_resolver_nodes_via_trait() {
        ...
        let resolver_nodes = NodeStorePort::query_resolver_nodes(&storage, &snap_uid).unwrap();
    ...
    fn insert_and_query_extraction_edges_via_trait() {
        ...
        let batch =
            EdgeStorePort::query_extraction_edges_batch(&storage, &snap_uid, 10, None).unwrap();
        assert_eq!(batch.len(), 1);
        assert_eq!(batch[0].edge_uid, "ee1");
        assert_eq!(batch[0].target_key, "./utils");
        assert_eq!(
            batch[0].edge_type,
            repo_graph_indexer::types::EdgeType::Imports
        );
        assert_eq!(
            batch[0].resolution,
            repo_graph_indexer::types::Resolution::Static
        );
    ...
    fn extraction_edges_cursor_pagination() {
        ...
        // First page: limit 2.
        let page1 =
            EdgeStorePort::query_extraction_edges_batch(&storage, &snap_uid, 2, None).unwrap();
        ...
        let page2 =
            EdgeStorePort::query_extraction_edges_batch(&storage, &snap_uid, 2, Some("ee_b"))
                .unwrap();
    ...
    fn insert_and_query_file_signals_via_trait() {
        ...
        let signals =
            FileSignalPort::query_file_signals_batch(&storage, &snap_uid, &["r1:src/a.ts".into()])
                .unwrap();
    ...
    fn query_file_signals_empty_uids_returns_empty() {
        let storage = setup();
        let signals = FileSignalPort::query_file_signals_batch(&storage, "snap1", &[]).unwrap();
    ...
    fn copy_forward_remaps_extraction_edge_source_node_uid() {
        ...
        let edges =
            EdgeStorePort::query_extraction_edges_batch(&storage, &snap2, 10, None).unwrap();
    ...
    fn copy_forward_copies_file_signals_and_versions() {
        ...
        let signals =
            FileSignalPort::query_file_signals_batch(&storage, &snap2, &["r1:src/a.ts".into()])
                .unwrap();
        assert_eq!(signals.len(), 1);
