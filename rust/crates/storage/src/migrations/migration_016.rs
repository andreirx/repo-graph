...
//!
//! Pure SQL: creates `surface_fs_mutations` and
//! `surface_fs_mutation_evidence` tables plus five indexes.
//! Mirrors `016-fs-mutations.ts`.
...
pub fn run(conn: &mut Connection) -> Result<(), StorageError> {
	...
	record_migration(conn, 16, "016-fs-mutations")?;
	Ok(())
}
