...
pub mod migration_025;
pub mod migration_026;
pub mod migration_027;
pub mod migration_028;
pub mod migration_029;
pub mod migration_030;
pub mod migration_031;
pub mod migration_032;

/// Apply all 32 storage migrations to the given connection.
///
...
/// version-gates the application of migrations 002 through 032.
///
...
pub fn run_migrations(conn: &mut Connection) -> Result<(), StorageError> {
    ...
    if max_version < 25 {
        migration_025::run(conn)?;
    }
    if max_version < 26 {
        migration_026::run(conn)?;
    }
    if max_version < 27 {
        migration_027::run(conn)?;
    }
    if max_version < 28 {
        migration_028::run(conn)?;
    }
    if max_version < 29 {
        migration_029::run(conn)?;
    }
    if max_version < 30 {
        migration_030::run(conn)?;
    }
    if max_version < 31 {
        migration_031::run(conn)?;
    }
    if max_version < 32 {
        migration_032::run(conn)?;
    }
...
mod tests {
    ...
    #[test]
    fn run_migrations_applies_all_thirty_two_migrations() {
        let mut conn = fresh_conn();
        run_migrations(&mut conn).expect("run all migrations");

        // schema_migrations table exists and contains rows 1..=32
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM schema_migrations", [], |row| {
                row.get(0)
            })
            .unwrap();
        assert_eq!(count, 32, "expected 32 migration rows after full run");
    }
    ...
    #[test]
    fn schema_migrations_records_versions_one_through_thirty_two_in_order() {
        let mut conn = fresh_conn();
        run_migrations(&mut conn).expect("run all migrations");
        ...
        let expected: Vec<(i64, &str)> = vec![
            ...
            (25, "025-contract-schema"),
            (26, "026-gcm-created-at"),
            (27, "027-freshness-provenance"),
            (28, "028-cache-semantics"),
            (29, "029-repair-orphan-fks"),
            (30, "030-resolved-call-aggregate"),
