...
pub fn backfill_obligation_ids(
    ...
    if changed {
        let serialized = serde_json::to_string(&parsed).map_err(|e| {
            malformed_requirement(declaration_uid, format!("re-serialize failed: {}", e))
        })?;
        Ok(Some(serialized))
    } else {
        Ok(None)
...
mod tests {
    ...
    fn assigns_obligation_id_to_entries_that_lack_one() {
        ...
        let parsed: serde_json::Value = serde_json::from_str(&updated.unwrap()).unwrap();
        let entries = parsed
            .get("verification")
    ...
    fn throws_when_verification_is_not_an_array() {
        let value =
            r#"{"req_id":"REQ-001","version":1,"objective":"x","verification":"not an array"}"#;
        let err = backfill_obligation_ids("decl-1", value).unwrap_err();
        assert!(matches!(err, StorageError::MalformedRequirement { .. }));
    }
    ...
    fn throws_when_a_verification_entry_is_not_an_object() {
        let value =
            r#"{"req_id":"REQ-001","version":1,"objective":"x","verification":["not an object"]}"#;
        let err = backfill_obligation_ids("decl-1", value).unwrap_err();
        assert!(matches!(err, StorageError::MalformedRequirement { .. }));
    }
    ...
    fn throws_when_a_verification_entry_has_empty_method() {
        let value =
            r#"{"req_id":"REQ-001","version":1,"objective":"x","verification":[{"method":""}]}"#;
        let err = backfill_obligation_ids("decl-1", value).unwrap_err();
        assert!(matches!(err, StorageError::MalformedRequirement { .. }));
    }
    ...
    fn generated_obligation_ids_have_uuid_v4_shape() {
        ...
        let value =
            r#"{"req_id":"REQ-001","version":1,"verification":[{"method":"arch_violations"}]}"#;
