...
fn dispatch_classify(input: &Value) -> Result<Value, String> {
    ...
    let file_signals: FileSignals = get_field(input, "fileSignals", "classify_unresolved_edge")?;
...
fn dispatch_blast_radius(input: &Value) -> Result<Value, String> {
    let category: UnresolvedEdgeCategory = get_field(input, "category", "derive_blast_radius")?;
    let basis_code: UnresolvedEdgeBasisCode = get_field(input, "basisCode", "derive_blast_radius")?;
...
fn dispatch_compute_matcher_key(input: &Value) -> Result<Value, String> {
    let mechanism: BoundaryMechanism = get_field(input, "mechanism", "compute_matcher_key")?;
...
fn dispatch_framework_boundary(input: &Value) -> Result<Value, String> {
    let target_key: String = get_field(input, "targetKey", "detect_framework_boundary")?;
    let category: UnresolvedEdgeCategory =
