...
pub fn compute_module_rollups(
    ...
    for (module_uid, is_test) in file_to_module.values() {
    ...
    for edge in &input.edges {
        ...
        if let Some(acc) = accumulators.get_mut(edge.source_module_uid.as_str()) {
            acc.outbound_modules.insert(edge.target_module_uid.clone());
        ...
        if let Some(acc) = accumulators.get_mut(edge.target_module_uid.as_str()) {
            acc.inbound_modules.insert(edge.source_module_uid.clone());
...
mod tests {
    ...
    fn counts_outbound_and_inbound_dependencies() {
        ...
        let app = result
            .iter()
            .find(|r| r.canonical_path == "packages/app")
            .unwrap();
        let core = result
            .iter()
            .find(|r| r.canonical_path == "packages/core")
            .unwrap();
        let util = result
            .iter()
            .find(|r| r.canonical_path == "packages/util")
            .unwrap();
    ...
    fn counts_violations_by_source_module() {
        ...
        let adapters = result
            .iter()
            .find(|r| r.canonical_path == "packages/adapters")
            .unwrap();
        let core = result
            .iter()
            .find(|r| r.canonical_path == "packages/core")
