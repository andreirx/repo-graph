...
use crate::types::{PackageDependencySet, RuntimeBuiltinsSet, TsconfigAliases};

/// True iff `name` appears in `deps.names`. Mirror of
/// `hasPackageDependency` from `signals.ts:44`.
pub(crate) fn has_package_dependency(deps: &PackageDependencySet, name: &str) -> bool {
...
pub(crate) fn has_runtime_builtin_module(builtins: &RuntimeBuiltinsSet, specifier: &str) -> bool {
...
pub(crate) fn matches_any_alias(specifier: &str, aliases: &TsconfigAliases) -> bool {
