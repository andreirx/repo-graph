...
use crate::types::{
    ClassifierVerdict, ImportBinding, UnresolvedEdgeBasisCode, UnresolvedEdgeCategory,
...
pub fn detect_framework_boundary(
    ...
    let first_method = method_part.split('.').next().unwrap_or(method_part);
    ...
    if file_has_express_import(import_bindings) && EXPRESS_RECEIVER_NAMES.contains(&receiver) {
