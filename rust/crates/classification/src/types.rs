...
impl FileSignals {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            tsconfig_aliases: TsconfigAliases {
                entries: Vec::new(),
            },
        }
    }
...
mod tests {
    ...
    fn boundary_role_serializes_lowercase() {
        assert_eq!(
            serde_json::to_string(&BoundaryRole::Provider).unwrap(),
            "\"provider\""
        );
        assert_eq!(
            serde_json::to_string(&BoundaryRole::Consumer).unwrap(),
            "\"consumer\""
        );
    }
    ...
    fn blast_radius_level_serializes_snake_case() {
        assert_eq!(
            serde_json::to_string(&BlastRadiusLevel::Low).unwrap(),
            "\"low\""
        );
        assert_eq!(
    ...
    fn is_calls_category_covers_all_four_calls_variants() {
        assert!(UnresolvedEdgeCategory::CallsThisWildcardMethodNeedsTypeInfo.is_calls_category());
        assert!(UnresolvedEdgeCategory::CallsThisMethodNeedsClassContext.is_calls_category());
        assert!(UnresolvedEdgeCategory::CallsObjMethodNeedsTypeInfo.is_calls_category());
        assert!(UnresolvedEdgeCategory::CallsFunctionAmbiguousOrMissing.is_calls_category());
    }
    ...
    fn matchable_provider_fact_serializes_fact_uid_as_top_level_sibling() {
        ...
        assert!(
            json.get("mechanism").is_some(),
            "mechanism must be top-level"
        );
        assert!(json.get("address").is_some(), "address must be top-level");
        assert!(
            json.get("handlerStableKey").is_some(),
            "handlerStableKey must be top-level (camelCase)"
        );
    }
