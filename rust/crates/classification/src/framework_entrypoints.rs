    ...
    for sym in exported_symbols {
        if sym.visibility.as_deref() == Some("export")
            ...
            && sym
                .subtype
                .as_deref()
                .is_some_and(|st| CALLABLE_SUBTYPES.contains(&st))
        {
...
mod tests {
    ...
    fn detects_exported_handler_with_aws_lambda_import() {
        let results = detect_lambda_entrypoints(&[lambda_import()], &[handler_function()]);
    ...
    fn detects_exported_const_handler() {
        let results = detect_lambda_entrypoints(&[lambda_import()], &[handler_variable()]);
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn detects_handler_with_powertools_import() {
        let results = detect_lambda_entrypoints(&[powertools_import()], &[handler_function()]);
        assert_eq!(results.len(), 1);
    }

    #[test]
    fn detects_handler_with_middy_import() {
        let results = detect_lambda_entrypoints(&[middy_import()], &[handler_function()]);
        assert_eq!(results.len(), 1);
    }
    ...
    fn does_not_detect_non_callable_export_named_handler_class() {
        let results = detect_lambda_entrypoints(&[lambda_import()], &[handler_class()]);
        assert!(results.is_empty(), "CLASS is not a callable subtype");
    }

    #[test]
    fn does_not_detect_type_alias_named_handler() {
        let results = detect_lambda_entrypoints(&[lambda_import()], &[handler_type_alias()]);
    ...
    fn returns_empty_when_handler_is_not_exported() {
        ...
        let results = detect_lambda_entrypoints(&[lambda_import()], &[sym]);
        assert!(results.is_empty());
    }
    ...
    fn returns_empty_when_no_conventional_handler_name() {
        let results = detect_lambda_entrypoints(&[lambda_import()], &[non_handler_export()]);
        assert!(results.is_empty());
    }
    ...
    fn exported_main_does_not_detect_p2_regression() {
        ...
        let results = detect_lambda_entrypoints(&[lambda_import()], &[main_fn]);
        assert!(
            results.is_empty(),
