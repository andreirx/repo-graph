...
//! for a small fixed set of strategies, avoiding trait objects.
//!
...
use crate::types::{
    BoundaryLinkCandidate, BoundaryMatchBasis, BoundaryMechanism, MatchableConsumerFact,
...
pub fn match_boundary_facts(
    ...
    for mech in [BoundaryMechanism::Http, BoundaryMechanism::CliCommand] {
        ...
        let candidates = match mech {
            BoundaryMechanism::Http => http_match(mech_providers, mech_consumers),
            BoundaryMechanism::CliCommand => cli_match(mech_providers, mech_consumers),
            _ => Vec::new(),
        };
...
fn http_match(
    ...
    let mut provider_by_path_only: HashMap<String, Vec<&MatchableProviderFact>> = HashMap::new();
                ...
                if let Some(path_matches) = provider_by_path_only.get(&path_only) {
                    ...
                    for p in path_matches {
                        candidates.push(BoundaryLinkCandidate {
                            ...
                            confidence: compute_http_confidence(c.fact.confidence, &seg_info) * 0.7,
                        });
                    }
                }
            }
...
mod tests {
    use super::*;
    use crate::types::{
        BoundaryConsumerBasis, BoundaryConsumerFact, BoundaryProviderBasis, BoundaryProviderFact,
    };
    use serde_json::json;
    ...
    fn http_key_uses_star_for_unknown_method() {
        let key = compute_matcher_key(BoundaryMechanism::Http, "/health", &Map::new());
        assert_eq!(key.unwrap(), "* /health");
    }

    #[test]
    fn http_key_uppercases_method() {
        let key = compute_matcher_key(BoundaryMechanism::Http, "/api", &http_meta("get"));
    ...
    fn cli_key_lowercases_and_trims() {
        let key = compute_matcher_key(BoundaryMechanism::CliCommand, "  Repo Add  ", &Map::new());
    ...
    fn unknown_mechanism_returns_none_for_matcher_key() {
        let key = compute_matcher_key(BoundaryMechanism::Grpc, "/some.Service/Method", &Map::new());
        assert!(key.is_none());
    }
}
