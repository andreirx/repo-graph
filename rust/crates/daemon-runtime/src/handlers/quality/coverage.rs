...
pub fn handle_coverage(state: &DaemonState, request: &Request) -> DispatchResult {
    ...
    // BOUNDED foreground patience — an honest, holder-named `Busy` transient under a concurrent write
    // pass instead of an unbounded block up to the 300s client-timeout SYMPTOM (the #2 mechanism).
    // No partial write on timeout — the measurement writes happen only after both guards are held.
