...
use super::*;
use crate::index_basis_probe::{classify_head_failure, parse_basis_outcome};
...
#[test]
fn partition_excludes_only_proven_exhaust_and_counts_it() {
    // SELF-POLLUTION-1 §2.2: rmap's OWN `map` sidecars (marker-confirmed), `.rgr/`,
    // and OS noise are excluded from the reader's changed set and counted; a user's
    // marker-less MAP.md (name collision) and a real source file are the reader's.
    ...
    // rmap-generated sidecar → marker present → excluded.
    write(
        "src/MAP.md",
        "<!-- generated by rmap map from snapshot snap-0; do not hand-edit -->\n# gen\n",
    );
    // user's hand-authored MAP.md → NO marker → the reader's change (honesty).
    write("docs/MAP.md", "# Hand-authored map\n");
    // OS noise + ROOT tool-state → excluded by name (definitional, no read).
    write(".DS_Store", "\0\0");
    write(".rgr/state.json", "{}");
    // review-4 finding 1: a `.rgr` component NESTED below the root is NOT rmap's
    // state root (rmap only writes `<repo_root>/.rgr/…`) → the reader's change,
    // never silently excluded.
    write("sub/.rgr/state.json", "{}");
    // a real source change → the reader's.
    write("src/lib.rs", "fn main() {}\n");
    ...
    let changed: Vec<String> = vec![
        ...
        ".DS_Store".into(),
        ".rgr/state.json".into(),
        "sub/.rgr/state.json".into(),
        "src/lib.rs".into(),
    ];

    let (readers, excluded, unreadable) = partition_changed(root, &changed);
    assert_eq!(
        excluded, 3,
        "generated MAP.md + .DS_Store + ROOT .rgr → 3 excluded (nested .rgr is NOT)"
    );
    assert_eq!(unreadable, 0, "all candidates were readable");
    let mut readers = readers;
    readers.sort();
    assert_eq!(
        readers,
        vec![
            "docs/MAP.md".to_string(),
            "src/lib.rs".to_string(),
            "sub/.rgr/state.json".to_string(),
        ],
        "marker-less user MAP.md + real source + nested .rgr are the reader's"
    );
}

#[test]
fn partition_distinguishes_absent_from_unreadable_sidecar() {
    // operator RULING 3: a sidecar-NAMED changed path we cannot READ is COUNTED as the
    // reader's change (conservative) — but the honest reason splits two ways:
    //   • NotFound (a deleted sidecar) → a genuine reader change, NOT "unreadable";
    //   • any other IO error (here: a *directory* at the path, so `read_to_string`
    //     fails with a non-NotFound error) → reader's change AND flagged unreadable,
    //     never silently asserted "not generated".
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    ...
    let changed = vec![
        "gone/MAP.md".to_string(), // never created → NotFound → Absent → reader, not unreadable
        "blocked/MAP.md".to_string(), // a dir → non-NotFound read error → unreadable reader
    ];
    let (mut readers, excluded, unreadable) = partition_changed(root, &changed);
    readers.sort();
    assert_eq!(excluded, 0, "neither is proven exhaust → nothing excluded");
    assert_eq!(
        unreadable, 1,
        "only the non-NotFound read failure is flagged unreadable, not the deleted one"
    );
    assert_eq!(
        readers,
        vec!["blocked/MAP.md".to_string(), "gone/MAP.md".to_string()],
        "both stay the reader's change (counted in M)"
    );
}
