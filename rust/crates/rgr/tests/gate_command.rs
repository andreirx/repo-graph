...
fn insert_requirement(
    ...
    conn.execute(
        ...
        rusqlite::params![
            uid,
            repo_uid,
            format!("{}:REQ:REQUIREMENT", repo_uid),
            value_json
        ],
    )
    .unwrap();
...
fn gate_malformed_requirement_aborts() {
    ...
    assert!(
        ...
        "no JSON on stdout for command error"
    );
...
fn gate_waiver_suppresses_fail() {
    ...
    assert!(
        basis["expires_at"].is_null(),
        "perpetual waiver has no expiry"
    );
}
...
fn gate_waiver_wrong_obligation_id_no_suppression() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "wrong obl_id => no suppression, exit 1"
    );
...
fn gate_expired_waiver_no_suppression() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "expired waiver => no suppression, exit 1"
    );
...
fn gate_pass_with_waiver_stays_pass() {
    ...
    assert_eq!(result["gate"]["counts"]["pass"], 1);
    assert_eq!(
        result["gate"]["counts"]["waived"], 0,
        "PASS must not count as waived"
    );
...
fn gate_strict_and_advisory_mutually_exclusive() {
    ...
    assert!(
        output.stdout.is_empty(),
        "no JSON on stdout for usage error"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
...
fn gate_coverage_threshold_missing_data() {
    ...
    assert_eq!(obls[0]["computed_verdict"], "MISSING_EVIDENCE");
    assert_eq!(
        obls[0]["evidence"]["reason"],
        "no coverage data for target path"
    );
}
...
fn gate_complexity_threshold_passes() {
    ...
    insert_complexity(
        ...
        &snap,
        "r1",
        "r1:src/core/service.ts:SYMBOL:helper",
        7.0,
    );
...
fn gate_complexity_threshold_fails() {
    ...
    insert_complexity(
        ...
        &snap,
        "r1",
        "r1:src/core/service.ts:SYMBOL:complex",
        15.0,
    );
...
fn gate_complexity_threshold_missing_data() {
    ...
    assert_eq!(
        obls[0]["evidence"]["reason"],
        "no complexity data for target path"
    );
}
...
fn gate_hotspot_threshold_evidence_shape() {
    ...
    assert!(
        ev.get("operator").is_none(),
        "operator should not be in evidence"
    );
    assert!(
        ev.get("functions_measured").is_none(),
        "functions_measured should not be in evidence"
    );
}
