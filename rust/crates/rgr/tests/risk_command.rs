...
fn risk_usage_error_no_args() {
    let output = Command::new(binary_path()).args(["risk"]).output().unwrap();
...
fn risk_repo_not_found() {
    ...
    let output = Command::new(binary_path())
        .args(["risk", db_path.to_str().unwrap(), "nonexistent-repo"])
        .output()
        .unwrap();
...
fn risk_envelope_contract() {
    ...
    for row in arr {
        ...
        assert!(
            row["hotspot_score"].is_number(),
            "row must have hotspot_score"
        );
        assert!(
            row["line_coverage"].is_number(),
            "row must have line_coverage"
        );
        assert!(
        ...
        assert!(
            row["sum_complexity"].is_number(),
            "row must have sum_complexity"
        );
    }
}
...
fn risk_coverage_missing_value_field_aborts() {
    ...
    assert_eq!(
        output.status.code(),
        Some(2),
        "missing value field must abort"
    );
    assert!(output.stdout.is_empty(), "stdout must be empty on error");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
...
fn risk_malformed_target_stable_key_aborts() {
    ...
    assert_eq!(
        output.status.code(),
        Some(2),
        "malformed target_stable_key must abort"
    );
    assert!(output.stdout.is_empty(), "stdout must be empty on error");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
