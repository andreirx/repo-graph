...
fn orient_focus_repeated_usage_error() {
    let (_r, _d, db) = build_indexed_repo();
    let db_str = db.to_str().unwrap();
    let output = run_cmd(&["orient", db_str, "r1", "--focus", "a", "--focus", "b"]);
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
...
fn orient_focus_flag_as_value_usage_error() {
    ...
    let (_r, _d, db) = build_indexed_repo();
    let db_str = db.to_str().unwrap();
    let output = run_cmd(&["orient", db_str, "r1", "--focus", "--bogus"]);
    assert_eq!(
        output.status.code(),
        Some(1),
...
fn orient_budget_flag_as_value_usage_error() {
    ...
    let (_r, _d, db) = build_indexed_repo();
    let db_str = db.to_str().unwrap();
    let output = run_cmd(&["orient", db_str, "r1", "--budget", "--focus", "x"]);
    assert_eq!(
        output.status.code(),
        Some(1),
...
fn orient_medium_budget_succeeds() {
    let (_r, _d, db) = build_indexed_repo();
    let output = run_cmd(&["orient", db.to_str().unwrap(), "r1", "--budget", "medium"]);
    assert_eq!(
        output.status.code(),
        Some(0),
        "stderr: {}",
