...
fn build_acyclic_db() -> (tempfile::TempDir, tempfile::TempDir, PathBuf) {
    ...
    .unwrap();
    std::fs::write(root.join("src/b/index.ts"), "export const foo = 1;\n").unwrap();
...
fn cycles_repo_not_found() {
    ...
    let output = Command::new(binary_path())
        .args(["cycles", db_path.to_str().unwrap(), "nonexistent-repo"])
        .output()
        .unwrap();
...
fn cycles_none_when_acyclic() {
    ...
    let output = Command::new(binary_path())
        .args(["cycles", db_path.to_str().unwrap(), "r1"])
        .output()
        .unwrap();
    ...
    assert_eq!(
        result["count"], 0,
        "acyclic graph should have 0 cycles, stdout: {}",
        stdout
    );
...
fn cycles_exact_results() {
    ...
    assert_eq!(result["count"], 1, "expected 1 cycle, got: {}", stdout);
