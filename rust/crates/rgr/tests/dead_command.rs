...
fn dead_usage_error() {
    let output = Command::new(binary_path()).args(["dead"]).output().unwrap();
...
fn dead_repo_not_found() {
    ...
    let output = Command::new(binary_path())
        .args(["dead", db_path.to_str().unwrap(), "nonexistent-repo"])
        .output()
        .unwrap();
...
fn dead_empty_when_all_referenced() {
    ...
    .unwrap();
    std::fs::write(root.join("src/b.ts"), "export function work() {}\n").unwrap();
    ...
    let output = Command::new(binary_path())
        .args(["dead", db_path.to_str().unwrap(), "r1", "SYMBOL"])
        .output()
        .unwrap();
...
fn dead_exact_results() {
    ...
    let output = Command::new(binary_path())
        .args(["dead", db_path.to_str().unwrap(), "r1"])
        .output()
        .unwrap();
