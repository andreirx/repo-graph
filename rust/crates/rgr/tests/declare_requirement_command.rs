...
fn base_args(db_str: &str) -> Vec<&str> {
    vec![
        "declare",
        "requirement",
        db_str,
...
fn declare_requirement_obligation_text_does_not_affect_identity() {
    ...
    assert_eq!(
        ...
        "obligation text change must not create new declaration"
    );
    assert_eq!(
        first_result["declaration_uid"],
        second_result["declaration_uid"],
...
fn declare_requirement_json_shape() {
    ...
    assert_eq!(result["kind"], "requirement");
    assert!(!result["declaration_uid"].as_str().unwrap().is_empty());
...
fn declare_requirement_missing_db() {
    ...
    assert!(
        ...
        "no JSON on stdout for storage error"
    );
}
