...
fn build_fixture_db() -> (tempfile::TempDir, tempfile::TempDir, PathBuf) {
	...
	.unwrap();
    std::fs::write(root.join("src/server.ts"), "export class Server {}\n").unwrap();
    std::fs::write(root.join("src/util.ts"), "export function helper() {}\n").unwrap();
...
fn callers_instantiates_only() {
    ...
    assert_eq!(
        result["count"], 1,
        "Server should have 1 INSTANTIATES caller, got: {}",
        result
    );
...
fn callees_instantiates_only() {
    ...
    assert_eq!(
        result["count"], 1,
        "main has 1 INSTANTIATES callee, got: {}",
        result
    );
...
fn callees_union_calls_and_instantiates() {
    ...
    assert_eq!(
        result["count"], 2,
        "main has 2 callees (CALLS + INSTANTIATES), got: {}",
        result
    );
