...
fn callers_ambiguous_symbol() {
    ...
    let root = dir.path();
    std::fs::create_dir_all(root.join("src")).unwrap();
    std::fs::write(root.join("src/a.ts"), "export function doWork() {}").unwrap();
    std::fs::write(root.join("src/b.ts"), "export function doWork() {}").unwrap();
    ...
    let output = Command::new(binary_path())
        .args(["callers", db_path.to_str().unwrap(), "ambig", "doWork"])
        .output()
        .unwrap();
...
fn callers_no_callers() {
    ...
    let output = Command::new(binary_path())
        .args(["callers", db_path.to_str().unwrap(), "r1", "unused"])
        .output()
        .unwrap();
...
fn callers_with_exact_results() {
    ...
    let output = Command::new(binary_path())
        .args(["callers", db_path.to_str().unwrap(), "r1", "serve"])
        .output()
        .unwrap();
    ...
    assert!(
        caller["stable_key"].as_str().unwrap().contains("index.ts"),
