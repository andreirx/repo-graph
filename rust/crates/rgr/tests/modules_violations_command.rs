...
fn modules_violations_stale_declaration() {
    ...
    assert_eq!(
        ...
        "expected 1 stale declaration, got: {}",
        result
    );
    let stale = result["results"]["stale_declarations"].as_array().unwrap();
    ...
    assert!(
        missing
            .iter()
            .any(|p| p.as_str().unwrap().contains("nonexistent")),
...
fn modules_violations_envelope_contract() {
    ...
    assert!(
        ...
        "diagnostics must be object"
    );
    let diag = &result["diagnostics"];
    assert!(
        diag["imports_edges_total"].is_number(),
        "imports_edges_total"
    );
    assert!(
        diag["imports_source_no_file"].is_number(),
        "imports_source_no_file"
    );
    assert!(
        diag["imports_target_no_file"].is_number(),
        "imports_target_no_file"
    );
    assert!(
        diag["imports_source_no_module"].is_number(),
        "imports_source_no_module"
    );
    assert!(
        diag["imports_target_no_module"].is_number(),
        "imports_target_no_module"
    );
    assert!(
        diag["imports_intra_module"].is_number(),
        "imports_intra_module"
    );
    assert!(
        diag["imports_cross_module"].is_number(),
...
fn modules_violations_diagnostics_healthy() {
    ...
    assert!(
        total >= 1,
        "should have at least 1 import edge, got {}",
        total
    );
    assert!(
        cross >= 1,
        "should have at least 1 cross-module import, got {}",
        cross
    );
}
