...
fn base_waiver_args(db_str: &str) -> Vec<&str> {
    vec![
        "declare",
        "waiver",
        db_str,
...
fn declare_waiver_missing_requirement_version() {
    ...
    assert!(
        stderr.contains("--requirement-version"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_waiver_reason_does_not_affect_identity() {
    ...
    assert_eq!(
        second_result["inserted"], false,
        "reason change must not create new declaration"
    );
    assert_eq!(
        first_result["declaration_uid"],
        second_result["declaration_uid"],
...
fn declare_waiver_optional_fields_do_not_affect_identity() {
    ...
    assert_eq!(
        second_result["inserted"], false,
        "optional fields must not affect identity"
    );
    assert_eq!(
        first_result["declaration_uid"],
        second_result["declaration_uid"],
...
fn declare_waiver_suppresses_gate() {
    ...
    assert_eq!(
        gate_before.status.code(),
        Some(1),
        "gate should FAIL without waiver"
    );
...
fn declare_waiver_expired_does_not_suppress() {
    ...
    assert_eq!(
        ...
        Some(1),
        "expired waiver must not suppress"
    );
