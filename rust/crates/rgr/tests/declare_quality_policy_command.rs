...
fn declare_quality_policy_missing_measurement() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("--measurement is required"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_missing_policy_kind() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("--policy-kind is required"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_missing_threshold() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("--threshold is required"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_invalid_measurement() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("unknown measurement kind"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_invalid_policy_kind() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("invalid --policy-kind"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_nan_threshold() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("threshold must be finite"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_invalid_scope_clause_format() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("invalid --scope-clause format"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_invalid_scope_clause_type() {
    ...
    let output = run_cmd(&[
        ...
        "15",
        "--scope-clause",
        "invalid_type:src/core",
    ]);
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("invalid scope clause type"),
        "stderr: {}",
        stderr
    );
}
...
fn declare_quality_policy_coverage_with_no_new_rejected() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "should reject no_new for coverage"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("incompatible") || stderr.contains("IncompatiblePolicyKind"),
...
fn declare_quality_policy_coverage_with_no_worsened_rejected() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "should reject no_worsened for coverage"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("incompatible") || stderr.contains("IncompatiblePolicyKind"),
...
fn declare_quality_policy_coverage_with_absolute_min_allowed() {
    ...
    assert_eq!(
        output.status.code(),
        Some(0),
        "absolute_min should be allowed for coverage: stderr={}",
        String::from_utf8_lossy(&output.stderr)
    );
}
