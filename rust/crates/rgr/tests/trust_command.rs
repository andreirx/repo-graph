...
fn trust_repo_not_found_exit_2() {
    ...
    let output = Command::new(binary_path())
        .args(["trust", db_path.to_str().unwrap(), "nonexistent-repo"])
        .output()
        .unwrap();
...
fn trust_success_produces_valid_report() {
    ...
    let output = Command::new(binary_path())
        .args(["trust", db_path.to_str().unwrap(), "classifier-repo"])
        .output()
        .unwrap();
    ...
    assert!(
        caveats
            .iter()
            .any(|c| c.as_str().unwrap().contains("Call-graph reliability")),
        "missing call-graph caveat"
    );
    assert!(
        caveats
            .iter()
            .any(|c| c.as_str().unwrap().contains("Cycle payloads")),
    ...
    let calls_cat = categories
        .iter()
        .find(|c| c["category"].as_str() == Some("calls_function_ambiguous_or_missing"));
