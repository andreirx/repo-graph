...
fn setup_waived_gate(db_str: &str) -> (String, String, String) {
    ...
    let b_uid = parse_json(&b)["declaration_uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let r_uid = parse_json(&r)["declaration_uid"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let w_uid = parse_json(&w)["declaration_uid"]
        .as_str()
        .unwrap()
        .to_string();
...
fn supersede_waiver_missing_reason() {
    ...
    let output = run_cmd(&["declare", "supersede", "waiver", db_str, &w_uid]);
    assert_eq!(output.status.code(), Some(1));
...
fn supersede_waiver_success() {
    ...
    assert_ne!(
        result["new_declaration_uid"].as_str().unwrap(),
        old_uid.as_str()
    );
...
fn supersede_waiver_gate_sees_new_reason() {
    ...
    assert_eq!(
        before["obligations"][0]["waiver_basis"]["reason"],
        "old reason"
    );
    ...
    assert_eq!(
        after["obligations"][0]["waiver_basis"]["reason"],
        "new reason after review"
    );
}
...
fn supersede_waiver_expired_restores_gate_failure() {
    ...
    let gate_after = run_cmd(&["gate", db_str, "r1"]);
    assert_eq!(
        gate_after.status.code(),
        Some(1),
        "expired waiver must not suppress gate"
    );
    let after = parse_json(&gate_after);
