...
fn orient_json_mode_returns_valid_envelope() {
    ...
    // ORIENT-LIVEGRAPH-IMPL (RISK-O-F): the top level is now the CoherenceEnvelope wrapper
    // `{ value, provenance, trust, freshness }`; the orient fields live under `value`.
    assert!(
    ...
    assert!(
        ...
        "value should have 'confidence'. stdout:\n{}",
        stdout
    );
    assert!(
