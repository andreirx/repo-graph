...
fn hotspots_repo_not_found() {
    ...
    let output = Command::new(binary_path())
        .args(["hotspots", db_path.to_str().unwrap(), "nonexistent-repo"])
        .output()
        .unwrap();
...
fn hotspots_score_is_lines_times_complexity() {
    ...
    assert_eq!(
        sum_complexity, 7,
        "sum_complexity should aggregate multiple symbols"
    );
}
...
fn hotspots_exclude_tests_removes_test_files() {
    ...
    let filtering = result
        .get("filtering")
        .expect("filtering field must be present");
...
fn hotspots_exclude_vendored_uses_segment_matching() {
    ...
    std::fs::write(
        repo_path.join("src/mydeps/util.ts"),
        "export const u = 1;\n",
    )
    .unwrap();
...
fn hotspots_combined_filters_with_overlap() {
    ...
    std::fs::write(
        repo_path.join("vendor/lib.test.ts"),
        "test('v', () => {});\n",
    )
    .unwrap();
