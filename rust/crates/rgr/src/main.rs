...
fn run_declare(args: &[String]) -> ExitCode {
	if args.is_empty() {
		...
		eprintln!("subcommands: boundary, requirement");
		return ExitCode::from(1);
	}
...
fn run_declare_requirement(args: &[String]) -> ExitCode {
	...
	/// Parse a required flag value. Returns `None` and prints an error
	/// if the flag is repeated, the value is missing, or the value
	...
	fn parse_flag_value<'a>(
		...
		*i += 1;
		if *i >= args.len() || args[*i].starts_with('-') {
			eprintln!("error: {} requires a value", flag_name);
			return None;
		}
		Some(args[*i].clone())
	}
