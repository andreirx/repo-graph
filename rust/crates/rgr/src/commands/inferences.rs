...
fn parse_inferences_list_args(args: &[String]) -> Result<(&Path, &str, Option<String>), String> {
    if args.len() < 2 {
        return Err("usage: rmap inferences list <db_path> <repo_uid> [--kind <kind>]".to_string());
    }
...
fn run_inferences_list(args: &[String]) -> ExitCode {
    ...
    let inferences = match storage
        .list_inferences_for_snapshot(&snapshot.snapshot_uid, kind_filter.as_deref())
    {
        Ok(i) => i,
        Err(e) => {
