...
fn run_contracts_list(args: &[String]) -> ExitCode {
	...
	let schemas = match storage.list_contract_schemas(&snapshot.snapshot_uid, kind_filter.as_deref()) {
		Ok(s) => s,
		Err(e) => {
...
fn run_contracts_elements(args: &[String]) -> ExitCode {
	...
	let schemas = match &file_filter {
		Some(path) => {
			match storage.get_schema_by_file(&snapshot.snapshot_uid, path) {
				Ok(Some(s)) => vec![s],
				Ok(None) => {
		...
		None => {
			match storage.list_contract_schemas(&snapshot.snapshot_uid, None) {
				Ok(s) => s,
				Err(e) => {
					eprintln!("query error: {}", e);
					return ExitCode::from(2);
	...
	for schema in schemas {
		let elements = match storage.list_elements_for_schema(&schema.schema_uid, kind_filter.as_deref()) {
			Ok(e) => e,
			Err(e) => {
				eprintln!("query error: {}", e);
				return ExitCode::from(2);
	...
	if let Some(ref f) = file_filter {
		extra.insert("filter_file".to_string(), serde_json::Value::String(f.clone()));
	}
