...
fn compute_sanity_metrics(
	...
	let max_owned: u64 = results.iter().map(|r| r.owned_file_count).max().unwrap_or(0);
	...
	let unowned_breakdown = compute_unowned_breakdown(
		storage,
		snapshot_uid,
		repo_uid,
		facts,
...
fn compute_unowned_breakdown(
	...
	let file_hashes = storage.query_file_version_hashes(snapshot_uid).unwrap_or_default();

	// Build set of owned file UIDs
	let owned_uids: HashSet<&str> = facts.owned_files()
		.iter()
		.map(|f| f.file_uid.as_str())
		.collect();

	// Build set of module root paths
	let module_roots: HashSet<&str> = results.iter()
	...
	let total_unowned = excluded_count + suppressed_test_count + true_gap_count;
	let true_gap_pct = if total_files > 0 {
		(true_gap_count as f64 / total_files as f64) * 100.0
	} else {
		0.0
	};
	let classified_pct = if total_unowned > 0 {
		100.0 // All unowned files are now classified
	} else {
		100.0
	};
