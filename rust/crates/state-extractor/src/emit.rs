...
use repo_graph_state_bindings::{
    ...
    BindingEntry, BindingTable, CalleePath, Driver, FsPathOrLogical, ImportView, Language,
    LogicalName, Provider, RepoUid, StableKey, ValidationError,
};
...
use crate::evidence::{LogicalNameSource, StateBoundaryEvidence, STATE_BOUNDARY_EVIDENCE_VERSION};
...
impl<'t> StateBoundaryEmitter<'t> {
    ...
    fn ensure_resource_node(
        ...
        if !self.resource_nodes.contains_key(&stable_key_str) {
            ...
            self.resource_insertion_order.push(stable_key_str.clone());
...
fn resolve_as_generic(
    ...
    match payload {
        ...
        CallsiteLogicalName::Fs(path) => {
            LogicalName::new(path.as_str().to_string()).map_err(|_| {
                EmitError::FsPayloadInNonFsSlot {
                    resource_kind: kind,
