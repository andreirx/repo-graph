...
STATUS: RATIFIED 2026-09-14 (human, option A); wording REVISED 2026-09-21 by the human (D-SD1-001, option "first sentence only" — the manager's pre-packet measurement `docs/audits/2026-09-21-seed-document-spike-1.md` showed the enclosing type's doc carries the whole effect, the identifier summary never raises a target score and lowers two, and a whole class doc dilutes precise matches and turns short members into class-doc echoes); SEED-DOCUMENT-1 is scheduled after the round-six queue (a re-embedding of every repo is the stated cost). The embedded document of a method chunk shall carry, besides its qualified name, doc comment and leading body lines, the FIRST SENTENCE of its enclosing type's doc comment, truncated at the slice-owned constant `repo-graph-seed::document::MAX_ENCLOSING_SENTENCE_CHARS` (160 characters at ratification; the value is the slice's, not frozen here; no identifier summary — measured to be harmful), so that a natural-language task query can reach the method that does the work; measured on the FRAKTAG persistence query with the floor and model unchanged and the SEED-CHUNK-2 control proofs byte-stable.

**Verification criterion:** `repo-graph-seed/src/document.rs` composition tests (pin the current form; to be extended: the first sentence is truncated at `MAX_ENCLOSING_SENTENCE_CHARS` on a char boundary, a method without a documented enclosing type embeds as today, the enclosing sentence precedes the method's own doc); the recorded rank/score table for the literal query `rmap find "where are conversations persisted to disk"` on FRAKTAG (createSession 0.274 at rank 14, logTurn 0.155, TreeStore.saveNode 0.046, ContentStore.create 0.130 vs floor 0.30; measured 2026-09-21 on a fresh isolated FRAKTAG index, HEAD 12eecd5) as the before; the same literal query as the after: `ConversationManager.createSession` (packages/engine/src/core/ConversationManager.ts:71) rendered above the floor in the top ten, the owning class `ConversationManager` (:61) still in the top three, 0 `[field]` rows; the controls keep their first row (`rmap find "how are content atoms hashed"` → `ContentStore.findByHash` packages/engine/src/core/ContentStore.ts:90 first; `rmap find "where are tree nodes saved to disk"` → `TreeStore.saveNode` packages/engine/src/core/TreeStore.ts:316 first); leveldb `rmap find "crash recovery"` keeps `leveldb::Iter::CorruptionError` (table/block.cc:242) first and `leveldb::RepairDB` (db/repair.cc:446) second (no leveldb class carries a stored doc comment, so its documents are byte-identical).

**Evidence (v0.18.0):** NOT MET — SEED-DOCUMENT-1 (its own root-caused slice; ratified position: after the round-six queue).

### RG-REQ-010-L11 — Cross-module concern hints, labeled Layer 3, beneath the deterministic facts
