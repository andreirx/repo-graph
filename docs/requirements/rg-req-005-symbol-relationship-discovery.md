...
In Python, `self.m()`/`cls.m()` inside class `C` shall resolve by walking `C` and its superclass closure breadth-first; the first depth with exactly one METHOD named `m` wins; a same-depth collision (a real MRO ambiguity) stays unresolved with its own category. Superclass facts come from the class's stored metadata; no other language's binding changes.

**Verification criterion:** resolver fixtures (`Base.run` / `Sub(Base)` calling `self.run()` → one CALLS edge to `Base.run`; two same-depth ancestors both defining `run` → unresolved); field: `explain BaseHandler.get_response` on django lists `WSGIHandler.__call__` (wsgi.py:124) and `ClientHandler.__call__` (client.py:186), not `ASGIHandler`, not exception.py's closure calls; leveldb and FRAKTAG edge counts byte-stable.
