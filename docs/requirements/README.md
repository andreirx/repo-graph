...
Status: DRAFT requirements set authored by the in-place manager on 2026-09-12 at the human's direction ("write the requirements for repo-graph — a new folder docs/requirements, one file per high-level requirement, containing all the related low-level requirements inside"). Independent requirements review: PERFORMED 2026-09-12 by Codex gpt-5.6-terra (standalone, read-only; record `reviews/2026-09-12-requirements-review-0-terra.txt`) — verdict REFINE; every per-ID finding is applied in this revision (splits: RG-REQ-004-L11, RG-REQ-006-L11/L12, RG-REQ-010-L11; PROPOSED markers on RG-REQ-006-L11, RG-REQ-008-L01, RG-REQ-010-L10/L11; fixture and rule corrections on RG-REQ-005-L01/L02; seven evidence labels lowered). Human approval of this revision: PENDING. Maturity: PROTOTYPE (requirements record).
...
| [RG-REQ-016](rg-req-016-architecture-preservation.md) | The accepted architecture is preserved by every change unless a change to it is explicitly authorized | 10 | 4 met · 3 partial · 2 unmet · 1 unknown |

**16 H requirements, 162 individually identified L requirements.** The "v0.18.0 reading" column is computed from each L's leading evidence label (a mixed line such as "OBSERVED MET as honesty; NOT MET as capability" counts by its first label — read the L); it is the manager's OBSERVED reading of the round-six audit and its root-cause record; it is not verification evidence and not acceptance. Each L carries a verification criterion naming the existing test, probe or protocol — or states NO EXISTING VERIFICATION and what to build.
...
- **Decisions for the human (each L names the problem and the options):** RG-REQ-011-L11 — the warm-`orient` latency bound (no defined benchmark: ratify a measured machine/corpus/percentile bound, publish measurements only, or self-calibrate per machine); RG-REQ-008-L01 — the docs discovery scope unfreezes DOCS-LIST-2's discovery freeze (ratify the stem/extension/`src/site` rule, keep it proposed pending a corpus probe, or retain the old scope); RG-REQ-006-L11 — the unique-suffix include fallback as Q3's second increment (roots only, or roots plus suffix); RG-REQ-009-L01's repository-declared generated exclusion and RG-REQ-010-L10 SEED-DOCUMENT-1 — ratify or keep in the proposed backlog.
- **Architecture preservation (human directive 2026-09-13):** RG-REQ-016 turns the standing obligation into a requirement; its evidence baseline is `docs/audits/2026-09-13-structure-reading.md` (rmap on itself). Two findings there need a decision: `quality-policy` → `storage` domain types (inward-rule violation; the fix changes a dependency edge) and the orphan `repo-graph-detectors` crate (wire / retire / record). RG-REQ-016 has not yet had an independent review.
