...
RATIFIED ORDER (human 2026-09-14, option A): Q1 → Q3 → Q4 → Q5 → Q6 → Q7 → Q8 → Q9 → ALIAS-SUSPICION-1 (the false "alias resolution suspected" reason on repo-graph/kafka/FRAKTAG, RC-11) → SEED-DOCUMENT-1 (RG-REQ-010-L10). Q2 shipped df08725. Ratified but UNSCHEDULED: CPP-INCLUDE-SUFFIX-1 (RG-REQ-006-L11; its own slice, after CPP-INCLUDE-ROOTS-1) and CONCERN-HINTS-1 (RG-REQ-010-L11; ROADMAP entry).
...
The round-seven fix queue (2026-09-23, after the human's rulings) maps onto: TOOLCHAIN-STALENESS-1 → RG-REQ-001-L06 (first: every extractor slice that follows bumps its affected extractor version constant and every resolver/indexer slice bumps `INDEXER_VERSION`, so existing stores re-index automatically on first use; D-STALE-SIGNAL-1, 2026-09-26) · CPP-ATTRIBUTE-MACRO-1A → RG-REQ-001-L01, RG-REQ-002-L11 · PYTHON-SUBMODULE-IMPORT-1 → RG-REQ-006-L04, RG-REQ-002-L11 · PYTHON-RECEIVER-BINDING-1 → RG-REQ-005-L02, RG-REQ-002-L11 · DOCS-UNREADABLE-DECODE-1 → RG-REQ-008-L05/L08 · TEST-EDGE-SCOPE-1A → RG-REQ-001-L07 · TEST-EDGE-SCOPE-1B → RG-REQ-004-L12 (the release gate) · TS-WORKSPACE-RESOLUTION-1 → RG-REQ-006-L04, RG-REQ-002-L11 · DEPS-GRADLE-CATALOG-1A/1B → RG-REQ-006-L13 · CPP-INCLUDE-BASENAME-1 → RG-REQ-006-L11, RG-REQ-002-L11.
...
- **Decisions for the human — CLOSED 2026-09-14 (each was put to the human self-contained; outcomes):** RG-REQ-011-L11 latency floor WITHDRAWN (no floor; the VISION's speed promise is algorithms, data structures and wiring, not a hardware benchmark); RG-REQ-008-L01 docs discovery scope RATIFIED as proposed (Q8 unblocked); RG-REQ-006-L11 include suffix fallback RATIFIED (≥2 segments, unique match; own slice CPP-INCLUDE-SUFFIX-1); RG-REQ-009-L01 refined (rmap's own heuristics now; repo-declared exclusions a possible future increment in one dedicated ecosystem-declarations module); RG-REQ-010-L10 seed document content RATIFIED (SEED-DOCUMENT-1 after the round-six queue); RG-REQ-010-L11 concern hints RATIFIED (CONCERN-HINTS-1, unscheduled). No L remains PROPOSED.
