...
**Verification criterion:** `doc-facts/src/discovery.rs` tests (existing: `discovers_readme_at_root`, `discovers_docs_directory_markdown`, `admits_txt_and_rst_under_docs_tree`, `skips_node_modules`; to add: stem rule, `.adoc`, `src/site`); field (CC-8, measured 2026-09-21 by walking all 17 local checkouts with the ratified rule under today's matching semantics — the stem is compared case-insensitively as the lowercase stem set applied to `README` already entails, the extension byte-for-byte as today's `ends_with(".md")` — headline counts exclude vendored and generated): MOVING — hadoop 22 → 556 (519 `src/site/markdown` guides, root `README.txt`/`BUILDING.txt`, 9 module `README.txt`, GNU-style `AUTHORS`/`ChangeLog`/`NEWS`; +1 vendored), django 616 → 621 (`README.rst`, `CONTRIBUTING.rst`, `AUTHORS`, `INSTALL`, `tests/README.rst`), buildroot 4 → 310 (73 `.adoc` manual files under `docs/`, 233 `board/**/readme.txt`), leveldb 7 → 9 (`AUTHORS`, `NEWS`), grpc-java 49 → 50 (`AUTHORS`), repo-graph 657 → 658 (`agent_docs/architecture.md`), gstreamer +8, vscode +3, OpenXcom +5, poco +2, FRAKTAG headline unchanged with +6 vendored (all under `packages/engine/fraktag-env/.../site-packages`); BYTE-STABLE — kafka, storybook, langchain4j, codegraph, spring-petclinic, mempalace. Not admitted under byte-for-byte extension matching: django `extras/README.TXT`, langchain4j `langchain4j-milvus/README.MD` (an open catalog question for the human, not decided by CC-8). Known residual of the ratified stem list: a bare `install` stem admits Debian packaging's `debian/install` (OpenXcom `install/debian/install`, a path list, not prose — 1 file in the 17 corpora).
