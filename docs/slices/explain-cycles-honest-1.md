<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/EXPLAIN-CYCLES-HONEST-1-INPUT-5.json",
  ...
  "candidatePaths": [
    ...
    "rust/crates/daemon-runtime/src/explain_coherence_served_tests.rs"
  ],
  ...
  "checks": [
    ...
    },
    {
      ...
      "method": {
        "kind": "command",
        "command": "(cd rust && cargo test -p repo-graph-daemon-runtime --lib explain_cycles_keeps_sqlite_primary_labelled_render_unsupported_when_livegraph_green 2>&1 | tee /tmp/ech-c17.txt | grep -E '^test result: ok\\. 1 passed; 0 failed') && ! grep -n 'fn explain_cycles_serves_live_cycles_from_livegraph(' rust/crates/daemon-runtime/src/explain_coherence_served_tests.rs",
        "cwd": ".",
        "environment": "candidate tree",
        "inputs": "the served-route e2e test in rust/crates/daemon-runtime/src/explain_coherence_served_tests.rs (pulled in via #[path] from explain_coherence_tests.rs): the OLD identity `explain_cycles_serves_live_cycles_from_livegraph` (asserted the leaf REBUILT from module_import_cycles, provenance {livegraph}, no fallback reason — the behaviour D-ECH-002 reverses) is RENAMED to `explain_cycles_keeps_sqlite_primary_labelled_render_unsupported_when_livegraph_green` with inverted assertions: with the cyclic LiveGraph resident and the cycles cert GREEN, the served EXPLAIN_CYCLES items are the SQLite primary's (the fixture's BOGUS_SQLITE_MODULE member PRESENT — proving no LiveGraph rebuild), `provenance.source == {Source::Sqlite}` and `provenance.fallback_reason == Some(CoherenceFallbackReason::LiveGraphRenderUnsupported)`; the section comment above it names A-1"
      },
      "expected": "exit 0: the served route keeps the SQLite primary (with whatever walk it carries) and labels the leaf honestly under a green LiveGraph; the old identity no longer exists (a test named for the reversed behaviour would be a false name)"
    },
    {
      "checkId": "ECH-C18",
      "obligationIds": [
      ...
      "method": {
        "kind": "command",
        "command": "(cd rust && cargo test -p repo-graph-storage --lib focus_cycle_reads_cancellable_return_cancelled_at_the_checkpoint_before_labeling 2>&1 | tee /tmp/ech-c18.txt | grep -E '^test result: ok\\. 1 passed; 0 failed') && test \"$(grep -c 'cancelled (client disconnected before cycle labeling)' rust/crates/storage/src/agent_impl.rs)\" = 2",
        "cwd": ".",
        "environment": "candidate tree",
        "inputs": "A-2 (ECH-IR-002): in `find_cycles_involving_path_cancellable` and `find_cycles_involving_module_cancellable` (rust/crates/storage/src/agent_impl.rs) one cooperative checkpoint `cancel()` immediately BEFORE `label_focus_cycles`, returning `AgentStorageError::new(<method>, \"cancelled (client disconnected before cycle labeling)\")` on Break — the read's LAST checkpoint; the NEW storage test `focus_cycle_reads_cancellable_return_cancelled_at_the_checkpoint_before_labeling` in rust/crates/storage/src/agent_cycle_labeling.rs (beside the two ECH-C03 tests, same ring fixture): (1) run each cancellable focus read with a COUNTING closure that always returns Continue and record N = the number of checkpoint calls; (2) run it again with a closure that returns Break exactly on call N (the last checkpoint, i.e. the one before labeling) and assert `Err` whose message contains `before cycle labeling` and that no labelled cycles were returned; (3) run with Break on call N+1 (never reached) and assert the same labelled result as the Continue run"
      },
      "expected": "exit 0: a client that disconnects after the filter loop abandons the read at the checkpoint before labeling (the cancellable-read contract of DAEMON-CANCEL-3 remains true on the new labeling path); both cancellable focus reads carry the checkpoint (exactly two occurrences of the message literal). Boundedness of the un-checkpointed labeling that follows a Continue: it is the same `label_cycles` the repo-level `find_module_cycles_cancellable` already runs un-checkpointed after ITS Tarjan (COHERENCE-3, ratified), over a SUBSET of that read's cycles — per-cycle checkpoints inside labeling for both reads are CANCEL-LABELING-1 (§7), not this slice"
    }
  ]
}
-->
...
| P-ECH-06 | The cancellable focus cycle reads honor a cooperative checkpoint immediately before labeling — the read's last checkpoint (DAEMON-CANCEL-3's cancellable-read contract stays true on the new labeling path, ECH-IR-002); the un-checkpointed labeling after a Continue is bounded by the repo-level `find_module_cycles_cancellable`'s already-ratified un-checkpointed labeling (COHERENCE-3) over a superset of these cycles; per-cycle checkpoints inside labeling for BOTH reads are CANCEL-LABELING-1 (§7) | ECH-C18 |
...
| A-2: served route (`build_explain_envelope`) with the cyclic LiveGraph resident and the cycles cert GREEN | the SQLite primary's items served (BOGUS_SQLITE_MODULE present), leaf `{sqlite}` + `LiveGraphRenderUnsupported` | `explain_cycles_keeps_sqlite_primary_labelled_render_unsupported_when_livegraph_green` (renamed from `explain_cycles_serves_live_cycles_from_livegraph`) |
| A-2: cancellable focus read, cancel fires at the last checkpoint (before labeling) | `Err` "cancelled (client disconnected before cycle labeling)", no labelled cycles | `focus_cycle_reads_cancellable_return_cancelled_at_the_checkpoint_before_labeling` |
...
| P-ECH-06 | a disconnected client's focus cycle read running the labeling with no checkpoint after the filter loop | ECH-C18 |
...
2. A-1 (§2.1 point 4): the two delegation tests FIRST (they fail on the applied candidate: the decorator's path-focus item carries no walk), then the port delegation, the explain leaf label, the removals and the comment corrections; A-2: rename the served-route test to its new identity with inverted assertions, add the pre-labeling checkpoint to both cancellable focus reads with its storage test; ECH-C13 → C14 → C16 → C17 → C18.
...
All eighteen checks pass; §2.3 holds on leveldb and vcmi; the five M-2 parity certificates pass unchanged and the two delegation tests prove the ring reaches the LiveGraph route by delegation; `cycles`/`orient` byte-identical; explain's other sections byte-identical; cost reported (no numeric bound).

## 7. Follow-ups (not this slice)

IMPORTS-WITNESS-UNION-1 (docs/TECH-DEBT.md 2026-09-19; D-ECH-002 direction 1, deferred by the human): the two engines are two witnesses of the import graph — SQLite (every language, repo-graph's own resolution stages) and the LiveGraph (TypeScript via scip-typescript, AST imports plus an in-memory tsconfig-alias / literal-dynamic overlay); a walk on the LiveGraph route, per-edge witness provenance for imports/cycles and the repo-level orient headline's decoration asymmetry all belong to it. TS-ALIAS-RESOLUTION-1 (D-ECH-002 direction 2, now — awaits ordering): a tsconfig `paths` alias resolution stage in the SQLite indexer (RG-REQ-006-L04) so that `FRAKTAG/packages/ui/src/components/ui/separator.tsx:4 import { cn } from "@/lib/utils"` (today `unresolved_edges` basis `SpecifierMatchesProjectAlias`, no module edge) resolves to `packages/ui/src/lib/utils.ts` on every route. Catalog corrections CC-1/CC-2/CC-3 queued in docs/assurance/RG-BOOTSTRAP/catalog-corrections.md. CANCEL-LABELING-1: per-cycle cooperative checkpoints inside `agent_cycle_labeling::label_cycles` (tracked-file load, classification, walk per SCC) for BOTH the repo-level and the focus cancellable reads — today both label un-checkpointed after their Tarjan (COHERENCE-3 / DAEMON-CANCEL-3); no requirement line owns query cancellation on client disconnect (a catalog completeness gap to queue as CC-4 at closeout).
...
- A-2 (2026-09-19; carried as INPUT-5; the fourth admission's review, codex gpt-5.6-terra cycle 1, refinement-required with two real findings): ECH-IR-001 (scope) — the served-route e2e test `explain_coherence_served_tests.rs::explain_cycles_serves_live_cycles_from_livegraph` asserts the behaviour A-1 reverses and its file was not in the candidate paths, so ECH-C15 (whole daemon-runtime unit suite: 776 passed, 1 failed) and ECH-C12 (exact path set) could not both pass — a manager allocation gap (the certificate grep of A-1 found the parity tests but not the served-route test); ECH-IR-002 (preservation) — the cancellable focus reads label after their checkpointed filter loop with no further checkpoint. Amendment: candidate path +1 (`rust/crates/daemon-runtime/src/explain_coherence_served_tests.rs`, 18 paths); §2.1 point 4 (e) rename + inverted assertions, (f) pre-labeling checkpoint; P-ECH-06 added; checks ECH-C17 (renamed served test; old identity gone) and ECH-C18 (cancellation regression test; both reads carry the checkpoint); ECH-C12's status list extended; §2.2 rows, §3, §5, §6, §7 (CANCEL-LABELING-1; CC-3) updated. The builder stopped correctly on the contradiction and ran no corpus proof (ECH-C05..C11 not-run), so the fifth admission re-runs every check.
