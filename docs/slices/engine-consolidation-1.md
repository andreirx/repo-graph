...
| **M-4** | **CLOSED 2026-07-18 (docs+comment; disposition follows from the ratified D-R5 supersession):** D-EC-6-C's LG-serve driver was "BEFORE M-6's first language drop"; with M-6's covered-language CALLS deletion SUPERSEDED (banner above — the rows are the reconciliation design's pipeline witness), no drop is scheduled, so the A-vs-D pick never becomes load-bearing. Path's pinned-SQLite Auto (the ratified CURRENT posture) simply persists; the LG fastpath stays behind explicit `--engine`, a deferred speed optimization exactly as the authoritative arm documents. Any future flip requires a D-EC-6 re-ratification. The §3.4-5 comment defect IS fixed (dispatch.rs path arm now states the pinned-SQLite truth and points at the authoritative arm). *Original row (for the record): under C, land the LG serve — A's cert-flip or D's degrade contract — before M-6's first drop; under B, record permanence and strike M-6.* | docs + comment fix (this disposition) | D-EC-6 (ratified as C); D-R5 supersession |
| **M-5** | **RECORDED OPTIONAL, UNSCHEDULED (2026-07-18):** its driver — "a per-language FC2a row drop cannot strand serving" — is removed by the D-R5 supersession (no drop scheduled; rows persist as the pipeline witness). Structural-proof cert-BUILDs retain independent value (cert cost/latency) and may be revived as ordinary perf work; no consolidation ordering depends on it. *Original row: PREREQ-2 for the covered subset: cert-BUILD sources become structural proofs (not SQLite compares); fallback paths audited so a per-language FC2a row drop cannot strand non-resident/stale/RED serving.* | (unscheduled) | M-2, M-3a, M-3b (all landed) |
