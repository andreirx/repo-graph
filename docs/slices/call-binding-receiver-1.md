<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/CALL-BINDING-RECEIVER-1-INPUT-6.json",
  ...
  "candidatePaths": [
    ...
    "rust/crates/repo-index/tests/call_binding_receiver.rs"
  ],
  ...
  "checks": [
    {
      ...
      "method": {
        "kind": "command",
        "command": "cargo test -p repo-graph-indexer --lib receiver_binding_ 2>&1 | tee /tmp/cbr-c01.txt | grep -E 'test result: ok\\. ([6-9]|[1-9][0-9]+) passed; 0 failed' && for t in receiver_binding_indirect_receiver_binds_to_its_declared_type receiver_binding_explicit_this_binds_to_enclosing_class receiver_binding_unknown_receiver_type_stays_unresolved_and_counted receiver_binding_receiverless_ambiguous_call_outside_class_stays_unresolved receiver_binding_indirect_receiver_never_binds_to_caller_itself receiver_binding_malformed_metadata_stays_unresolved receiver_binding_qualified_receiver_type_never_matches_a_shorter_container; do grep -qE \"^test .*$t .* ok$\" /tmp/cbr-c01.txt || { echo \"MISSING $t\"; exit 1; }; done",
        "cwd": "rust",
        "environment": "candidate tree",
        "inputs": "the receiver_binding_ resolver test family in rust/crates/indexer/src/resolver.rs: the seven named tests (section 2.5 (a)–(e), the F-CBR-004 malformed-metadata regression, and the F-CBR-005 qualified-receiver regression) plus any further regression test the family gains"
      },
      "expected": "exit 0: at least six receiver_binding_ tests pass with 0 failed, AND each of the seven NAMED tests is listed as ok — the count is a lower bound so a regression test added under review does not turn a green family red; a missing named test or any failure fails the check."
    },
    {
      "checkId": "CBR-C02",
      "obligationIds": [
    ...
    },
    {
      ...
      "method": {
        "kind": "command",
        "command": "cargo test -p repo-graph-repo-index --test call_binding_receiver 2>&1 | tee /tmp/cbr-fixture.txt | grep -E 'test result: ok\\.' && ! grep -E ' 0 passed' /tmp/cbr-fixture.txt",
        "cwd": "rust",
        "environment": "candidate tree; the test writes its own C++ fixture (class A { void run(); }; class B { A* a_; X* x_; void run(); } with a_->run(), this->run(), x_->run() where X is not indexed) into a tempfile directory and indexes it through repo-graph-repo-index's integration harness (the crate that depends on cpp-extractor, indexer and storage — see cpp_sb_1_integration.rs and the index_*_fixture_* tests), then queries the resulting store",
        "inputs": "rust/crates/repo-index/tests/call_binding_receiver.rs (new; decision D-CBR-C06-HOME-1). The indexer's rust/crates/indexer/tests/call_binding_receiver.rs remains the honest resolver-seam unit test and is NOT this check."
      },
      "expected": "exit 0 with at least one test passed: on the fixture INDEXED end-to-end (extractor → indexer → store), callers A::run == [B::run]; callees B::run contains A::run and B::run (the this->run() self-call is the ONE legitimate self-loop) and nothing else; the x_->run() call is an unresolved_edges row with a calls_* category (counted); the corpus assertion — CALLS self-loops whose metadata carries a receiver other than \"this\" — returns 0."
    },
    {
      "checkId": "CBR-C07",
      "obligationIds": [
    ...
    },
    {
      ...
      "method": {
        "kind": "command",
        "command": "git diff --check && test \"$(git status --short | grep -vE '^(\\?\\?| M) (rust/crates/cpp-extractor/src/extractor.rs|rust/crates/indexer/src/resolver.rs|rust/crates/indexer/tests/call_binding_receiver.rs|rust/crates/repo-index/tests/call_binding_receiver.rs)$' | wc -l | tr -d ' ')\" = 0 && git status --short",
        "cwd": ".",
        "environment": "candidate tree at hand-off",
        "inputs": "git status / git diff --check"
      },
      "expected": "exit 0: whitespace-clean and ONLY the four candidatePaths changed or added. Any other path (a fixture directory, a golden file, a docs edit) is out of scope — STOP and report rather than widen."
    }
  ]
}
-->
...
2. **A receiver-type binding stage in the shared resolver.** In `resolve_call_target`, BEFORE the bare-name singleton at :942 and gated on the edge carrying `receiverType`: look up `<receiverType>::<target_key>` in the qualified-name/stable-key index; on a unique hit bind — where a hit means the candidate's qualified CONTAINER equals the receiver type EXACTLY, or, when the receiver type is written unqualified, exactly one candidate's container has that terminal segment; a qualified receiver type never matches a shorter container by suffix (`a::Foo` does not evidence a global `Foo` — reviewer finding F-CBR-005; RG-REQ-005-L02: never widen what counts as evidence), guarded by the regression test `receiver_binding_qualified_receiver_type_never_matches_a_shorter_container`; on a miss with IMPLEMENTS edges available, walk the receiver type's bases (BFS, first depth with exactly one hit); otherwise fall through to the existing stages.
...
- OC-5 (2026-09-14; carried as INPUT-6 with review): reviewer finding F-CBR-005 (fourth admission cycle 3) — `container_matches` accepted a reverse suffix match (receiverType `a::Foo` vs container `Foo`), evidence-widening; §2.2 now states the exact-container rule and names the regression test `receiver_binding_qualified_receiver_type_never_matches_a_shorter_container`; CBR-C01's count becomes a lower bound (≥ 6, 0 failed) with each named test asserted present, so review-driven regression tests no longer turn the family's oracle red.
- OC-4 (2026-09-14, decision D-CBR-C06-HOME-1; carried as INPUT-5 with review because it adds a candidate path): CBR-C06 re-homed to `rust/crates/repo-index/tests/call_binding_receiver.rs` (the indexer crate cannot index C++ — reviewer finding F-CBR-003 exposed a mocked test); CBR-C01 counts six `receiver_binding_` tests (the F-CBR-004 malformed-metadata regression); CBR-C19 and candidatePaths list four paths.
- OC-3 (2026-09-14; carried as INPUT-4): CBR-C17's text half stripped only the ring line; its overflow line `(+ N more members in this cycle)` is walk-derived too (N differed 1 vs 2 between two identical indexes — second-admission cycle 2, reported `failed`, member sets identical). Both walk-derived lines are now stripped; the member-set comparison from `cycles --json` remains the proof. Same cause as CYCLES-WALK-DETERMINISM-1.
