<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/CALL-BINDING-RECEIVER-1-INPUT-5.json",
  ...
  "checks": [
    {
      ...
      "method": {
        "kind": "command",
        "command": "cargo test -p repo-graph-indexer --lib receiver_binding_ 2>&1 | grep -E 'test result: ok\\. 6 passed; 0 failed'",
        "cwd": "rust",
        "environment": "candidate tree",
        "inputs": "the six write-first resolver tests named in section 2.5: receiver_binding_indirect_receiver_binds_to_its_declared_type, receiver_binding_explicit_this_binds_to_enclosing_class, receiver_binding_unknown_receiver_type_stays_unresolved_and_counted, receiver_binding_receiverless_ambiguous_call_outside_class_stays_unresolved, receiver_binding_indirect_receiver_never_binds_to_caller_itself, receiver_binding_malformed_metadata_stays_unresolved (F-CBR-004 regression: a malformed metadata carrier is Unreadable — never eligible for the enclosing-class preference)"
      },
      "expected": "exit 0: exactly 6 tests with the receiver_binding_ prefix run and pass (grep matches the 'test result' line for that filter). Fewer than 6 means a section-2.5 test is missing; any failure fails the check."
    },
    {
      "checkId": "CBR-C02",
      "obligationIds": [
