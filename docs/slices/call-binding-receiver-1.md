<!-- requirements-assurance-implementation-v1
{
  ...
  "checks": [
    ...
    },
    {
      ...
      "method": {
        "kind": "command",
        "command": "( cd '/Users/apple/Documents/APLICATII BIJUTERIE/legacy-codebases/leveldb' && export RMAP_STATE_ROOT=/private/tmp/CALL-BINDING-RECEIVER-1-ldb RMAP_SOCKET_PATH=/private/tmp/CALL-BINDING-RECEIVER-1-ldb/d.sock RMAP_TRANSPORT=stdio RMAP_AUTO_ENRICH=off RMAP_AUTO_RETENTION=off && PATH=\"/Users/apple/Documents/APLICATII BIJUTERIE/repo-graph/rust/target/release:$PATH\" \"/Users/apple/Documents/APLICATII BIJUTERIE/repo-graph/rust/target/release/rmap\" callees leveldb::DBImpl::Recover --json ) > /tmp/cbr-callees.json 2>/dev/null && python3 -c \"import json\nd=json.load(open('/tmp/cbr-callees.json'))\nrows=[]\nst=[d]\nwhile st:\n o=st.pop()\n if isinstance(o,dict):\n  if 'qualified_name' in o: rows.append(o['qualified_name'])\n  st.extend(o.values())\n elif isinstance(o,list): st.extend(o)\nassert 'leveldb::VersionSet::Recover' in rows, rows\nassert 'leveldb::DBImpl::Recover' not in rows, rows\nprint('callees ok', len(rows))\"",
        "cwd": ".",
        "environment": "the candidate rmap on the CBR-C10 isolated leveldb index",
        "inputs": "the captured callees --json document"
      },
      "expected": "exit 0: callees of leveldb::DBImpl::Recover include leveldb::VersionSet::Recover (the `versions_->Recover(save_manifest)` call at db_impl.cc:324 now binds through the declared type of the field versions_ : VersionSet*) and no longer include leveldb::DBImpl::Recover itself."
    },
    {
      "checkId": "CBR-C13",
      "obligationIds": [
    ...
    },
    {
      ...
      "expected": "exit 0: `rmap cycles` on leveldb is byte-identical before and after (import cycles do not read CALLS; size-1 self-loops were never cycles — scc.rs self_loop_not_counted_as_cycle_by_size)."
    },
    {
      "checkId": "CBR-C18",
      "obligationIds": [
...
6. **Outward proof on leveldb (fresh isolated indexes, `RMAP_TRANSPORT=stdio`; before-binary from a git worktree of the recorded base revision):** `callers leveldb::DBImpl::Recover --json` = exactly {`leveldb::DB::Open`} rendered at db/db_impl.cc:1503 (the caller node's line — today's renderer; leveldb has ONE call site, db_impl.cc:1511) (CBR-C11); `callees` includes `leveldb::VersionSet::Recover` and excludes itself (CBR-C12); receiver-bearing self-loops = 0 and resolved+unresolved call sites CONSERVED before→after (CBR-C10); the DB::Open→Recover row has left the unresolved set by binding (CBR-C13); trust's headline before/after REPORTED with attribution and its --json consistent with the store (CBR-C14); `explain` Callers = Open (db) db/db_impl.cc:1503 (CBR-C15). Before-fingerprint on the retained v0.18.0 store copy: 155 self-loops / 3261 resolved / 6152 unresolved / DB::Open→Recover unresolved at 1511 / self-loop row line 324 receiver versions_ (CBR-C08).
...
Follow-ups filed, NOT in this slice: CALLERS-ANCHOR-1 (callers/callees rows anchor the CALL SITE — the edge's line — per RG-REQ-005-L08; today the caller node's line renders); the `dead` substrate has no surface while `dead` is refused (RG-REQ-013-L07) — nothing to file.
