<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/EXPLAIN-TYPE-SECTIONS-1-INPUT-3.json",
...
- INPUT-3 (2026-09-20): oracle correction OC-1 — ETS-C10 invoked `explain --budget small`, a tier explain's CLI does not have (`medium|large` or `--full`); the tiers are now medium (default), large, --full. Text-only (docs/MANAGER.md § Oracle corrections); no allocation, check-id, requirement or path change. Found by the first admission's builder (12/13 green; the product was correct, the command was not).
