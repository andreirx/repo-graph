<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/ALIAS-SUSPICION-1-INPUT-3.json",
...
3. **The trigger.** `alias_isolated_modules(&[ModuleForSuspicionCheck]) -> Vec<(String, u64)>` = zero-connectivity modules with `alias_unresolved_imports ≥ 1`, sorted by path; `detect_alias_resolution_suspicion(&alias_isolated)` is triggered iff non-empty, with ONE reason string PER module, `alias_isolated_module=<n> <path>` (count first, path last — A-2/D-AS1-002: reversible for every path; the comma-joined form of INPUT-2 could render a false name for a path containing a comma). The `>= 3` literal and the `suspicious_zero_connectivity_modules=N` reason are retired; the two tests that pinned them are RENAMED to their new identities (§2.2). `compute_import_graph_reliability` / `compute_change_impact_reliability` keep their boolean input.
...
5. **Two rendered lines (rgr trust.rs).** Suspicious Modules section (heading and basis sentence unchanged): `  - <module> — N imports through a project alias did not resolve` (singular `1 import`) for evidenced modules, `  - <module>` otherwise. Downgrades block (`render_downgrades`): the alias bullet becomes reader-facing — `  - Alias resolution suspected — <module> (N imports through a project alias did not resolve)[; …]` parsed from each `alias_isolated_module=<n> <path>` reason (A-2) — with NO `alias_resolution_suspicion:` key prefix on human output (RG-REQ-002-L08: the raw rule key is an internal-diagnostic label; capital `A` opens the bullet sentence). The machine key stays in `--json` (`triggered_downgrades.alias_resolution_suspicion`, `reasons: ["alias_isolated_module=<n> <path>", …]` (one string per module)) and the axis clause (`Import-graph: LOW (alias resolution suspected; …)`) is unchanged (already humanized). The other three downgrade bullets keep their raw `key:` prefix (untouched — a separate pre-existing L08 residual, §8). The axis lines and the humanizer are untouched.
6. **Parity.** The two `rules__detect-alias-resolution__*` fixtures carry the new input (`aliasIsolatedModules: [{path, count}]`) and the per-module reason `alias_isolated_module=55 packages/ui`.
...
| one zero-connectivity module with alias count ≥ 1 | triggered; reason `alias_isolated_module=<n> <path>` | AS-C02 `alias_suspicion_fires_on_one_alias_isolated_module` |
...
| two evidenced modules | two reasons, one per module, path order | AS-C02 `alias_suspicion_reason_names_each_module_with_its_count_in_path_order` |
| module path `packages/ui,legacy (old)` | reason and rendered line carry the path verbatim | AS-C02 `alias_suspicion_reason_survives_commas_parentheses_and_spaces_in_the_module_path`; AS-C05 `downgrades_block_renders_a_module_path_containing_a_comma_verbatim` |
...
- INPUT-2 (2026-09-21): A-1 (candidate path added: `trust-parity-fixtures/report__diagnostics-with-calls/expected.json`) + OC-1 (AS-C09 corrected; AS-C08/C10 assert the footnote stays; §0 and P-AS-02 state the deps-footnote movement); ledger docs/assurance/ALIAS-SUSPICION-1/oracle-corrections.md (pinned). Its admission: 11/11 checks green; the review raised D-AS-REASON-ENCODING (F-AS-001, correctness): the comma-joined reason grammar of this allocation could render a false module name for a path containing a comma.
- INPUT-3 (2026-09-21): A-2 / D-AS1-002 (operator resolution, human may override): one reason per module, `alias_isolated_module=<n> <path>`; two tests bound; the parity fixture's expected reason; candidate-2.patch continues with that one code change.
