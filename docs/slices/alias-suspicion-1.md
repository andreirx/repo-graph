<!-- requirements-assurance-implementation-v1
{
  ...
  "baselinePath": "docs/requirements/baselines/ALIAS-SUSPICION-1-INPUT-1.json",
...
**Changes:** none allocated. REPORTED consequences: on kafka the Import-graph and Change-impact reasons lose the false alias clause (their LOW levels stay for their true causes); on FRAKTAG and repo-graph the alias reason stays and is now scoped to one module each; `deps list`'s "resolution downgraded on this index" marker (dispatch.rs:6946-6961 reads the overlay token, which is unchanged) does not move on any of the four corpora (AS-C09 asserts kafka byte-identical).
...
| P-AS-02 | the axis token `alias_resolution_suspicion`, the humanizer, the overlay flag and every inheriting surface (orient, deps list, stats) are untouched — only the trigger's evidence and two rendered lines change | AS-C06 (diff --quiet); AS-C09 (deps list byte-identical) |
...
- INPUT-1 (2026-09-21): first admission baseline — this allocation, generated from the three parent requirement files' source closure under RG-BOOTSTRAP-INPUT-5.
