<!-- requirements-assurance-implementation-v1
{
  ...
  "checks": [
    ...
    },
    {
      ...
      "owner": "builder",
      "method": { "kind": "command", "command": "export RMAP_STATE_ROOT=/private/tmp/TRUST-MODULE-EDGES-1-rg RMAP_SOCKET_PATH=/private/tmp/TRUST-MODULE-EDGES-1-rg/d.sock RMAP_TRANSPORT=stdio RMAP_AUTO_ENRICH=off RMAP_AUTO_RETENTION=off && rust/target/release/rmap trust > /tmp/tme-trust.txt && ! grep -q 'Suspicious Modules (zero connectivity)' /tmp/tme-trust.txt && ! grep -q 'alias resolution suspected' /tmp/tme-trust.txt && grep -Eq 'Import-graph: LOW \\([0-9]+ unresolved imports\\)' /tmp/tme-trust.txt && grep -Fq 'Change-impact: LOW (import graph reliability low)' /tmp/tme-trust.txt && rust/target/release/rmap trust --json > /tmp/tme-trustj.json && python3 -c \"import json,sys\nd=json.load(open('/tmp/tme-trustj.json'))\nimp=False; ar=False; at=False\nst=[(None,d)]\nwhile st:\n key,o=st.pop()\n if isinstance(o,dict):\n  r=o.get('reasons')\n  if isinstance(r,list):\n   if 'import_graph_reliability_low' in r: imp=True\n   if 'alias_resolution_suspicion' in r: ar=True\n  if key=='alias_resolution_suspicion' and o.get('triggered') is True: at=True\n  for k,v in o.items(): st.append((k,v))\n elif isinstance(o,list):\n  for x in o: st.append((key,x))\nsys.exit(0 if (imp and (not ar) and (not at)) else 1)\"", "cwd": ".", "environment": "freshly built candidate rmap at rust/target/release/rmap; /private/tmp/TRUST-MODULE-EDGES-1-rg is an isolated throwaway COPY of ~/repo-graph-retained/audit-v0.18.0/repo-graph (copied, never the original) served with RMAP_TRANSPORT=stdio and auto passes off; operator real registry untouched", "inputs": "the copied repo-graph snapshot store; the captured human `rmap trust` text and the captured `rmap trust --json` document" },
      "expected": "exit 0 requires every && condition to hold. On the candidate's HUMAN `rmap trust` output (reader frame, not machine tokens — the axis reasons are humanized by agent::reliability::humanize_reason): `! grep` proves the 'Suspicious Modules (zero connectivity)' section is absent (was 48 rows) and the reader phrase 'alias resolution suspected' (the humanized alias reason) is absent; `grep -E` proves the reader line 'Import-graph: LOW (<n> unresolved imports)' is present (the level stays LOW while unresolved imports > 0 — only the false alias reason is gone); `grep -F` proves the exact reader line 'Change-impact: LOW (import graph reliability low)' is present (the humanized form of the import_graph_reliability_low token that is now Change-impact's sole reason). Then the MACHINE reason is verified in `rmap trust --json`: the python3 walk recurses the whole envelope (robust to nesting) and exits 0 only when some reliability-axis `reasons` array contains the machine token 'import_graph_reliability_low' AND no `reasons` array anywhere contains 'alias_resolution_suspicion' AND no object sitting under the key 'alias_resolution_suspicion' has \"triggered\": true (the key itself is always present in triggered_downgrades, so a substring search would be wrong; the walk distinguishes the ever-present key from a triggered downgrade or an axis reason). Any surviving Suspicious section, any surviving humanized alias phrase, a missing/changed Import-graph or Change-impact reader line, an absent machine token, or a resurgent alias reason/trigger flips a grep or the python exit and the chain exits non-zero. (Per-module fan equality to modules deps and cross-repo fan>0 are the named-record inspection TME-C06.)"
    },
    {
      "checkId": "TME-C06",
      "obligationIds": ["RG-REQ-009-L02", "RG-REQ-004-L01"],
      "owner": "builder",
      "method": { "kind": "inspection", "subject": "the candidate rmap's per-module fan counts on isolated throwaway COPIES of the retained repo-graph, kafka, hadoop, FRAKTAG and vcmi stores (~/repo-graph-retained/audit-v0.18.0/*, copied never opened)", "criterion": "on the repo-graph copy, every module in the captured `rmap trust --json` modules[] has fan_in and fan_out equal to that module's per-module counts in the captured `rmap modules deps --json` (records /tmp/tme-trust-repo-graph.json and /tmp/tme-deps-repo-graph.json), and rust/crates/agent shows fan_out >= 1 and fan_in >= 2; on the kafka, hadoop and FRAKTAG copies the count of module candidates with fan_in>0 OR fan_out>0 is > 0 (it was 0 at v0.18.0); on the vcmi copy the 11 already-connected rows remain connected; no verdict LEVEL is asserted to change, and any observed level change is reported as a finding rather than tuned away", "inputs": "the five throwaway store copies; the per-repo captured `rmap trust --json` and `rmap modules deps --json` records" },
      "expected": "the inspection finds repo-graph trust fan_in/fan_out equal modules deps per module (agent connected), kafka/hadoop/FRAKTAG each have > 0 connected module candidates, and vcmi's 11 remain connected. A per-module fan mismatch on repo-graph, a zero connected-count on any of kafka/hadoop/FRAKTAG, or a lost vcmi connection fails the check."
    },
    {
      "checkId": "TME-C07",
...
**Changes (pre-authorised):** `trust` on repo-graph loses its "Suspicious Modules (zero connectivity)" section (48 → 0) and the `alias_resolution_suspicion` downgrade; the human Import-graph line moves from `LOW (alias resolution suspected; 4903 unresolved imports)` to `LOW (4903 unresolved imports)` and the human Change-impact line from `LOW (alias resolution suspected)` to `LOW (import graph reliability low)` — the reader-frame text the `agent::reliability::humanize_reason` humanizer emits for the `import_graph_reliability_low` machine token that `rules.rs::compute_change_impact_reliability` now leaves as the sole Change-impact reason once the alias reason is gone (the machine token in `trust --json` stays `import_graph_reliability_low`; the human surface renders it with underscores replaced by spaces). The LEVEL does not change while unresolved imports > 0 (`rules.rs:215-243`); kafka/hadoop/FRAKTAG lose their false lists likewise; `orient`'s reliability caveat and `assess` inherit the wording.
