...
NEXT: the bootstrap re-baseline and Q7 COMPLEXITY-SCOPE-1 (both below).

RG-BOOTSTRAP-INPUT-4 re-baseline LANDED (2026-09-20, 821c59a): repo-graph CLAUDE.md gains "Relay Builder and Reviewer Rules" (isolation; retained roots are copies; smallest corpus; the builder runs the packet's checks, the operator runs the workspace suite/dogfood; cleanup; code-under-analysis examples) and the End-of-Slice Test bullet names the operator's gate (CC-5); CC-2/CC-3 applied as dated notes; codex gpt-5.6-terra standalone pass ACCEPT; approval 4. CC-1 (RG-REQ-004-L02 "never persisted" vs `create_module_edges`) and CC-4 (no L owns query cancellation on disconnect) await the human.

Q7 COMPLEXITY-SCOPE-1 SHIPPED (2026-09-20; builder claude-opus-4-8, reviewer codex gpt-5.6-terra):
`orient`'s complexity centers now rank the repository's OWN production code and say what they set aside (RC-9 never worked:
`files.is_generated` was hard-coded false at every construction site, the vendored predicate lived in an adapter crate the
core could not call and lacked `dependencies`, and the aggregator read neither `is_test` nor `is_generated`). The fix is
vertical: index-time `is_generated` from a FROZEN first-8-lines marker sniff (`@generated`, `Automatically generated`,
`DO NOT EDIT`, `made by GNU Bison` — `generated by` was measured and DROPPED: it flags leveldb `table/block.cc:5`
`// Decodes the blocks generated by block_builder.cc.`, hand-written) at the one site with content in hand
(`indexer/src/generated_sniff.rs`, wired at `orchestrator.rs:551`), the copy-forward site carrying the STORED flag (the
`files` upsert's ON CONFLICT would have erased it on every refresh — `is_generated_survives_no_change_refresh`); ONE vendored
predicate in the inner crate (`classification/src/vendored_path.rs`, `VENDORED_SEGMENTS` + `dependencies` + `contrib`;
daemon consumers forward from it); the storage read carries `is_test`/`is_generated` with its row SET unchanged (the
LiveGraph complexity certificate compares that set — its outcome is a provenance LABEL, never a value substitution); the
agent-side filter after the read and before truncation, `high_complexity_count` from the kept rows, the excluded count
carried as `ComplexityScope { Production { excluded_count } | All }` (JSON `scope: {kind, excluded_count}`); `--include-all`
threaded CLI → untyped orient params → daemon → `orient_cancellable` (usage error with `--focus`; the 5-arg `orient()`
wrapper unchanged); both renderers print `N generated/vendored/test symbols excluded — --include-all` (omitted when nothing
was excluded — a line reading the same on every repo answers nothing; malformed `scope` → `complexity scope unreadable on
this snapshot`). CODE UNDER ANALYSIS: poco — BEFORE `Complexity centers: dependencies/pcre2/src/pcre2_match.c — match (cx
1580), dependencies/pcre2/src/pcre2_dfa_match.c — internal_dfa_match (cx 839), dependencies/pcre2/src/pcre2_compile.c —
parse_regex (cx 749) (+386 more …)` (`match` at pcre2_match.c:687 — vendored; the first non-vendored entry was the Bison
`yyparse` of `Data/SQLParser/src/parser/bison_parser.cpp`, line 1 `/* A Bison parser, made by GNU Bison 3.8.2.  */`); AFTER
`Complexity centers: Foundation/src/Windows1251Encoding.cpp — convert (cx 345), Foundation/src/Windows1250Encoding.cpp —
convert (cx 225), Foundation/src/Latin2Encoding.cpp — convert (cx 177) (+168 more above threshold — rmap hotspots; 218
generated/vendored/test symbols excluded — --include-all)`; `docs list` `14 documents` → `13 documents` + `+1 vendored doc
(excluded; …)` for `dependencies/README.md` (RG-REQ-008-L04, pre-authorised). codegraph — BEFORE #1
`codegraph-kernel/grammars/lua/parser.c — ts_lex (cx 322)` (line 1 `/* Automatically @generated by tree-sitter */`); AFTER
`src/mcp/tools.ts — handleExplore (cx 256), src/extraction/tree-sitter.ts — extractCall (cx 244), src/ui-server/api/steps.ts
— buildSteps (cx 211) (+176 more …; 12 generated/vendored/test symbols excluded — --include-all)`, `--include-all` restores
`ts_lex` first; the 10 excluded test rows are fixtures such as `__tests__/fixtures/displacement-ts/src/pipeline/ingest.ts:13
export function ingestRecords` (`is_test = 1`). repo-graph — `rust/crates/storage/tests/parity.rs:159 fn dispatch_op (cx 92)`
leaves the production view (`--include-all --full` shows it). leveldb — byte-identical complexity line, hotspots and docs
(nothing to exclude; 0 markers). Store-derived seam (python recomputes the predicate over each store) equals the JSON
evidence on every corpus: codegraph old index 191/10/181, fresh 191/12/179 (5 generated files); poco 389/218/171 (2);
repo-graph 191/2/189 (0); leveldb 8/0/8 (0). RESIDUALS (one example each, stated, not hidden): hand-written tree-sitter
external scanners stay ranked (`codegraph-kernel/grammars/scala/scanner.c:295 tree_sitter_scala_external_scanner_scan (cx
83)` — `#include` header, no marker, no fact); poco's 788 CppUnit `testsuite/` files carry `is_test = 0` before and after
(`Foundation/testsuite/src/TestApp.cpp:240 int main` cx 42 — the gtest-only structural detector, RG-REQ-001-L07, preserved)
→ IS-TEST-CPPUNIT-1; the pre-existing `deps` segment reads repo-graph's own `rust/crates/module-queries/src/deps/compose.rs`
`compose_dependency_summaries (cx 36)` as vendored → VENDORED-SEGMENT-DEPS-1 (a predicate decision for the human);
codegraph `__tests__/generated-flag-index.test.ts:5` quotes the `DO NOT EDIT` banner in a doc comment and is flagged
generated (harmless: also `is_test`); vscode `multilineModelWeights.ts` stays ranked (no marker, no fact). Seed corpus
narrows by the newly flagged files only (codegraph 5, poco 2; `seed_impl.rs:81` unchanged). GATES: ALL-GATES-GREEN (scripts/repo-graph-gates.sh, exit 0: fmt, clippy --workspace --all-targets -D warnings, per-crate tests incl. daemon-runtime, consolidation witness, release bins, isolated dogfood; on the unchanged accepted candidate; committed bytes = the accepted candidate, proven by diff digest 95b176b7…; commit 7cbcb82). TALLY: 2
admissions, 2 baselines (INPUT-1 accepted 23/23 at PREP cycle 2 after one manager oracle defect — CS-C16's porcelain pattern
missed the unstaged ` M ` form — plus four line-number drifts corrected by the author; INPUT-2 = OC-1/OC-2 under D-CS-001,
accepted 23/23 at PREP-2 cycle 1 with zero findings), 1 reviewer decision D-CS-001 (operator resolution option 1, the L's own
"a file distinguishable by no persisted fact stays ranked"; human may override), 3 document + 3 implementation cycles: first
admission cycle 1 — the builder delivered the whole seven-part fix in 45 minutes, 14/16 with CS-C12/C13 failing ONLY on the
manager's directory-prefix greps (verified on the stores), the reviewer reproduced five checks and raised D-CS-001 +
F-CS-001 (a stale aggregator comment claiming `count_*` was still read); second admission cycle 1 16/16 with F-CS-002
(`support.rs` forwarded one of the two allocated names), cycle 2 16/16 accepted, six checks reproduced, both abstractions
judged earned. 4 manager interpretations (one builder evidence with an extra `findings` field; three reviewer subject/decision
shapes — never a changed outcome). A nit the allocation caused: forwarding `VENDORED_SEGMENTS` from `support.rs` has no
in-crate consumer and needed `#[allow(unused_imports)]` — SUPPORT-REEXPORT-1 (drop the dormant re-export; one line).
LESSONS: oracles assert the persisted FACT the contract promises, never a directory the fact usually lives under; measure a
content-marker list on the real corpora before freezing it; a heredoc must be the LAST element of a `&&` chain; do not
